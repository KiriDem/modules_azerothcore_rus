...
void IndividualProgression::UpdateAccountReputation(uint32 factionId, uint32 accountId, Player* player)
{
    ...
    ChatHandler(player->GetSession()).PSendSysMessage("Текущая {} Реп = {} ({})", factionId, curRep, factionName);
    ChatHandler(player->GetSession()).PSendSysMessage("Самый высокий {} Реп = {} ({})", factionId, newRep, factionName);
    ...
    if (location == "onyxia40" || location == "onyxia")
    {
        if (player->HasItemCount(ITEM_DRAKEFIRE_AMULET))
        {
            for (GroupReference* itr = group->GetFirstMember(); itr; itr = itr->next())
            {
                ...
                {
                    ChatHandler(player->GetSession()).PSendSysMessage("|cff00ffff{}|r необходим как минимум 50-й уровень.", member->GetName());
                    continue;
                }
                ...
                if (!member->HasItemCount(ITEM_DRAKEFIRE_AMULET))
                {
                    ...
                    {
                        ChatHandler(player->GetSession()).PSendSysMessage("|cff00ffff{}|r иметь Амулет Драконьего Огня в банке.", member->GetName());
                        continue;
                    }

                    member->AddItem(ITEM_DRAKEFIRE_AMULET, 1);
                    ChatHandler(player->GetSession()).PSendSysMessage("|cff00ffff{}|r получил Амулет Драконьего Пламени.", member->GetName());
                }
            }
            return;
        }
        ...
        {
            ChatHandler(player->GetSession()).PSendSysMessage("Чтобы использовать эту команду, у вас в инвентаре должен быть Амулет драконьего пламени.");
            return;
        }
    }
    else if (location == "bt" || location == "blacktemple")
    {
        if (player->HasItemCount(ITEM_MEDALLION_OF_KARABOR) || player->HasItemCount(ITEM_BLESSED_MEDALLION_OF_KARABOR))
        {
            for (GroupReference* itr = group->GetFirstMember(); itr; itr = itr->next())
            {
                ...
                {
                    ChatHandler(player->GetSession()).PSendSysMessage("|cff00ffff{}|r необходим как минимум 70-й уровень.", member->GetName());
                    continue;
                }

                if (isBeforeProgression(member, PROGRESSION_TBC_TIER_2))
                {
                    ChatHandler(player->GetSession()).PSendSysMessage("|cff00ffff{}|r требуется уровень прогресса 10 (TBC Tier 2).", member->GetName());
                    continue;
                }
                ...
                if (!member->HasItemCount(ITEM_MEDALLION_OF_KARABOR) && !member->HasItemCount(ITEM_BLESSED_MEDALLION_OF_KARABOR))
                {
                    ...
                    {
                        ChatHandler(player->GetSession()).PSendSysMessage("|cff00ffff{}|r иметь Медальон Карабора в своем банке.", member->GetName());
                        continue;
                    }

                    member->AddItem(ITEM_MEDALLION_OF_KARABOR, 1);
                    ChatHandler(player->GetSession()).PSendSysMessage("|cff00ffff{}|r получил Медальон Карабора.", member->GetName());
                }
            }
            return;
        }
        ...
        {
            ChatHandler(player->GetSession()).PSendSysMessage("YЧтобы воспользоваться этой командой, у вас в инвентаре должен быть Медальон Карабора.");
            return;
        }
    }
}
