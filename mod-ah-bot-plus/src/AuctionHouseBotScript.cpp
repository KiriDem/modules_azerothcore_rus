...
public:
    ...
    static bool HandleAHBotUpdateCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        handler->PSendSysMessage("AuctionHouseBot: Обновление аукционного дома...");
        AuctionHouseBot::instance()->Update();
        LOG_INFO("module", "AuctionHouseBot: Auction House Updated.");
        handler->PSendSysMessage("AuctionHouseBot: Аукционный дом обновлен.");
        return true;
    }
    ...
    static bool HandleAHBotReloadCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        handler->PSendSysMessage("AuctionHouseBot: Перезагрузка конфигурации...");
        ...
        handler->PSendSysMessage("AuctionHouseBot: Конфигурация перезагружена.");        
        return true;
    }
    ...
    static bool HandleAHBotEmptyCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        handler->PSendSysMessage("AuctionHouseBot: Очистка аукциона...");
        ...
        handler->PSendSysMessage("AuctionHouseBot: Аукционы очищенны.");
        return true;
    }

    static bool HandleAHBotHelpCommand(ChatHandler* handler, char const* /*args*/)
    {
        handler->PSendSysMessage("Команды AuctionHouseBot:");
        handler->PSendSysMessage("  .ahbot reload - Перезагружает конфигурацию");
        handler->PSendSysMessage("  .ahbot empty  - Удаляет все аукционные лоты AuctionHouseBot.");
        handler->PSendSysMessage("  .ahbot update - Выполняет цикл обновления");
        return true;
    }
};
