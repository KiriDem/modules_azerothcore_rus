...
public:
    ...
    void OnPlayerLogin(Player *player)
    {
        ...
        if (BFEnableModule && BFAnnounceModule)
            ChatHandler(player->GetSession()).SendSysMessage("На этом сервере работает модуль |cff4CFF00BufferNPC |r.");
    }
};
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (BuffCureRes && player->HasAura(15007))
        {
            ...
            res << "Аура смерти больше не тяготеет над тобой, " << PlayerName << ". Береги себя!";
