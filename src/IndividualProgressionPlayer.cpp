...
public:
    ...
    void OnPlayerLogin(Player* player) override
    {
        ...
        if (sIndividualProgression->enabled)
        {
            ...
            ChatHandler(player->GetSession()).SendSysMessage("|cff00ff00Индивидуальный прогресс: |cffccccccenabled|r");
        }
    }
        ...
        if (player->getClass() == CLASS_DRUID)
        {
            if (spell->GetSpellInfo()->SchoolMask == 8) // Nature
            {
                ...
                for (uint16 spellId : Druid_SM_8)
                {
                    ...
                    {
                        ChatHandler(player->GetSession()).PSendSysMessage("Это заклинание недоступно в WotLK.");
                        spell->cancel();
                        break;
                    }
                }
    ...
    bool OnPlayerBeforeTeleport(Player* player, uint32 mapid, float x, float y, float z, float /*orientation*/, uint32 /*options*/, Unit* /*target*/) override
    {
        ...
        {
            ChatHandler(player->GetSession()).PSendSysMessage("Требуемый уровень прогресса = |cff00ffff{}|r", PROGRESSION_MOLTEN_CORE);
            return false;
        }
        ...
        if (mapid == MAP_ZUL_GURUB)
        {
            ...
            {
                ChatHandler(player->GetSession()).PSendSysMessage("Требуемый уровень прогресса = |cff00ffff{}|r", REQUIRED_ZG_PROGRESSION);
                return false;
            }
        }
        if (mapid == MAP_AHN_QIRAJ_TEMPLE && !sIndividualProgression->hasPassedProgression(player, PROGRESSION_PRE_AQ))
        {
            ChatHandler(player->GetSession()).PSendSysMessage("Требуемый уровень прогресса = |cff00ffff{}|r", PROGRESSION_PRE_AQ);
            return false;
        }
        ...
        if (mapid == MAP_OUTLAND)
        {
            if (!sIndividualProgression->hasPassedProgression(player, PROGRESSION_PRE_TBC))
            {
                ...
                {
                    ChatHandler(player->GetSession()).PSendSysMessage("Требуемый уровень прогресса = |cff00ffff{}|r", sIndividualProgression->tbcRacesProgressionLevel);
                    return false;
                }
                else
            ...
            {
                ChatHandler(player->GetSession()).PSendSysMessage("Требуемый уровень прогресса = |cff00ffff{}|r", PROGRESSION_TBC_TIER_4);
                return false;
            }
        }
        if (mapid == MAP_ZUL_AMAN)
        {
            ...
            {
                ChatHandler(player->GetSession()).PSendSysMessage("Требуемый уровень прогресса = |cff00ffff{}|r", REQUIRED_ZA_PROGRESSION);
                return false;
            }
        }
        if (mapid == MAP_NORTHREND && !sIndividualProgression->hasPassedProgression(player, PROGRESSION_TBC_TIER_5))
        {
            ...
            ChatHandler(player->GetSession()).PSendSysMessage("Требуемый уровень прогресса = |cff00ffff{}|r", PROGRESSION_TBC_TIER_5);
            return false;
        }
        if (mapid == MAP_ULDUAR && !sIndividualProgression->hasPassedProgression(player, PROGRESSION_WOTLK_TIER_1))
        {
            ChatHandler(player->GetSession()).PSendSysMessage("Требуемый уровень прогресса = |cff00ffff{}|r", PROGRESSION_WOTLK_TIER_1);
            return false;
        }
        ...
        {
            ChatHandler(player->GetSession()).PSendSysMessage("Требуемый уровень прогресса = |cff00ffff{}|r", PROGRESSION_WOTLK_TIER_2);
            return false;
        }
        ...
        {
            ChatHandler(player->GetSession()).PSendSysMessage("Требуемый уровень прогресса = |cff00ffff{}|r", PROGRESSION_WOTLK_TIER_3);
            return false;
        }
        if (mapid == MAP_THE_RUBY_SANCTUM && !sIndividualProgression->hasPassedProgression(player, PROGRESSION_WOTLK_TIER_4))
        {
            ChatHandler(player->GetSession()).PSendSysMessage("Требуемый уровень прогресса = |cff00ffff{}|r", PROGRESSION_WOTLK_TIER_4);
            return false;
        }
        ...
        if (mapid == MAP_TEMPEST_KEEP)
        {
            ...
            {
                ChatHandler(player->GetSession()).PSendSysMessage("Требуемый уровень прогресса = |cff00ffff{}|r", PROGRESSION_TBC_TIER_1);
                return false;
            }
 			else if (!player->HasItemCount(ITEM_TEMPEST_KEY))
            {
                ChatHandler(player->GetSession()).PSendSysMessage("Чтобы войти в Око, вы должны обладать Ключом Бури.");
                return false;
            }
 			else if (player->GetQuestStatus(TRIAL_MAGTHERIDON) != QUEST_STATUS_REWARDED)
            {
                ChatHandler(player->GetSession()).PSendSysMessage("Чтобы попасть в Око, вам необходимо выполнить задание Испытание наару: Магтеридон.");
                return false;
            }
        }
        if (mapid == MAP_COILFANG_SERPENTSHRINE_CAVERN)
        {
            ...
            {
                ChatHandler(player->GetSession()).PSendSysMessage("Чтобы попасть в Змеиное святилище, вам необходимо выполнить задание Дубина Кар'деша.");
                return false;
            }
        }
        if (mapid == MAP_THE_BATTLE_FOR_MOUNT_HYJAL)
        {
            ...
            {
                ChatHandler(player->GetSession()).PSendSysMessage("Требуемый уровень прогресса = |cff00ffff{}|r", PROGRESSION_TBC_TIER_2);
                return false;
            }
 			else if (player->GetQuestStatus(VIALS_OF_ETERNITY) != QUEST_STATUS_REWARDED)
            {
                ChatHandler(player->GetSession()).PSendSysMessage("Чтобы попасть на поле боя Битва на горе Хиджал, необходимо выполнить задание Флаконы вечности.");
                return false;
            }
        }
        if (mapid == MAP_BLACK_TEMPLE)
        {
            ...
            {
                ChatHandler(player->GetSession()).PSendSysMessage("Чтобы войти в Черный храм, вы должны обладать Медальоном Карабора.");
                return false;
            }
        }
    ...
    void OnPlayerCompleteQuest(Player* player, Quest const* quest) override
    {
        ...
        if (sIndividualProgression->questMoneyAtLevelCap)
        {
            ...
            if (moneyRew > 0)
            {
                ...
                if (gold > 0)
                    ChatHandler(player->GetSession()).PSendSysMessage("Полученно {} Золота, {} Серебра.", gold, silv);
                else
                    ChatHandler(player->GetSession()).PSendSysMessage("Полученно {} Серебра.", silv);
            }
        }
    ...
    bool OnPlayerCanGroupInvite(Player* player, std::string& membername) override
    {
        ...
        if (sIndividualProgression->enforceGroupRules) // enforceGroupRules enabled
        {
            if (sIndividualProgression->isNormalAccount(player))
            {
                if (sIndividualProgression->isBotAccount(otherPlayer))
                {
                    if (!sIndividualProgression->hasPassedProgression(player, PROGRESSION_PRE_TBC)) // player is in vanilla
                    {
                        ...
                        {
                            ChatHandler(player->GetSession()).SendSysMessage("|cff00ff00Включено соблюдение правил группы: |cffccccccуровень игрока слишком большой.|r");
                            return false;
                        }
                    }
                    else if (!sIndividualProgression->hasPassedProgression(player, PROGRESSION_TBC_TIER_5)) // player is in TBC
                    {
                        ...
                        {
                            ChatHandler(player->GetSession()).SendSysMessage("|cff00ff00Включено соблюдение правил группы: |cffccccccуровень игрока слишком низкий или слишком высокий.|r");
                            return false;
                        }
                    }
                    else // player is in WotLK
                    {
                        ...
                        {
                            ChatHandler(player->GetSession()).SendSysMessage("|cff00ff00Включено соблюдение правил группы: |cffccccccуровень игрока слишком низкий.|r");
                            return false;
                        }
                    }
                }
            ...
            else // if (sIndividualProgression->isExcludedAccount(player))
            {
                ...
                {
                    ChatHandler(player->GetSession()).SendSysMessage("|cff00ff00Включено соблюдение правил группы: |cffccccccэтот игрок не является ботом и не имеет заблокированного аккаунта.|r");
                    return false;
                }
            }
        }
