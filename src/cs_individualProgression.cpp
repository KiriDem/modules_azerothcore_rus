...
public:
    ...
    static bool HandleGetIndividualProgressionCommand(ChatHandler* handler, Optional<PlayerIdentifier> player)
    {
        ...
        if (!target)
        {
            handler->PSendSysMessage("Игрок |cff00ffff{}|r не в сети.", playername);
            return false;
        }

        uint32 progressionLevel = sIndividualProgression->GetPlayerProgressionFromQuests(target);

        handler->PSendSysMessage("Уровень прогресса для |cff00ffff{}|r = |cff00ffff{}|r", playername, progressionLevel);
        return true;
    }
    ...
    static bool HandleSetIndividualProgressionCommand(ChatHandler* handler, Optional<PlayerIdentifier> player, uint32 progressionLevel)
    {
        ...
        {
            handler->SendSysMessage("Недопустимый уровень прогресса.");
            return false;
        }
        ...
        handler->PSendSysMessage("Обновленный уровень прогресса для |cff00ffff{}|r = |cff00ffff{}|r", playername, progressionLevel);
        return true;
    }
    ...
    static bool HandleSetBotIndividualProgressionCommand(ChatHandler* handler)
    {
        ...
        if (!group)
        {
            handler->SendSysMessage("Чтобы использовать эту команду, вы должны состоять в группе.");
            return false;
        }
        ...
        handler->PSendSysMessage("Обновлен уровень прогрессии для всех ботов RND. = |cff00ffff{}|r", currentState);
        return true;
    }
    ...
    static bool HandleSetRepIndividualProgressionCommand(ChatHandler* handler)
    {
        ...
        {
            handler->SendSysMessage("Команда .ip setrep в данный момент отключена.");
            return false;
        }
        
        if (!group)
        {
            ChatHandler(player->GetSession()).PSendSysMessage("Чтобы использовать эту команду, вы должны состоять в группе.");
            return false;
        }
    ...
    static bool HandleTeleIndividualProgressionCommand(ChatHandler* handler, Optional<PlayerIdentifier> player, std::string location)
    {
        ...
        {
            handler->PSendSysMessage("|cff00ffff{}|r не является допустимым местом телепортации.", location);
            return false;
        }
        ...
        if (location == "naxx" || location == "naxx40")
        {
 			if (target->GetLevel() <= 70 && ((progressionLevel < PROGRESSION_TBC_TIER_5) || sIndividualProgression->isExcludedAccount(target)))
            {
                ...
                {
                    handler->PSendSysMessage("|cff00ffff{}|r не выполнил задание для получения доступа в Наксрамас.", playername);
                    return false;
                }
            }
            else
            {
                handler->PSendSysMessage("|cff00ffff{}|r телепортироваться запрещено |cff00ffff{}|r.", playername, location);
                return false;
            }
        }
        else if (location == "onyxia" || location == "onyxia40")
        {
            if (target->GetLevel() <= 70 && ((progressionLevel < PROGRESSION_TBC_TIER_5) || sIndividualProgression->isExcludedAccount(target)))
            {
                ...
                {
                    handler->PSendSysMessage("|cff00ffff{}|r не имеет амулета Драконье пламя.", playername);
                    return false;
                }
            }
            else
    ...
    static bool HandleAttuneIndividualProgressionCommand(ChatHandler* handler, std::string location)
    {
        ...
        {
            handler->PSendSysMessage("|cff00ffff{}|r не является допустимой настройкой.", location);
            return false;
        }
    ...
    static bool HandlePVPIndividualProgressionCommand(ChatHandler* handler, Optional<PlayerIdentifier> player)
    {
        ...
        if (nextPvPRank == -1)
        {
            handler->PSendSysMessage("|cff00ffff{}|r достиг высшего PvP-ранга и в данный момент имеет {} почетных убийств.", playername, kills);
            return true;
        }
        else if (highestTitle == -1)
        {
            handler->PSendSysMessage("|cff00ffff{}|r не получил никакого PvP-ранга и в данный момент и имеет {} почетных побед.", playername, kills);
            handler->PSendSysMessage("|cff00ffff{}|r Требуется еще {} почетных убийств для получения 1-го ранга.", playername, killsToNextRank);
            return true;
        }
        else
        {
            if (teamId == TEAM_ALLIANCE)
            {
                handler->PSendSysMessage("PvP-ранг для |cff00ffff{}|r = |cff00ffff{}|r", playername, highestTitle + ALLIANCE_PVP_RANK_OFFSET);
                handler->PSendSysMessage("|cff00ffff{}|r нужно еще {} почетных убийств для получения ранга {}.", playername, killsToNextRank, nextPvPRank + ALLIANCE_PVP_RANK_OFFSET);
                return true;
            }
            else // teamId == TEAM_HORDE
            {
                handler->PSendSysMessage("PvP-ранг для |cff00ffff{}|r = |cff00ffff{}|r", playername, highestTitle - HORDE_PVP_RANK_OFFSET);
                handler->PSendSysMessage("|cff00ffff{}|r нужно еще {} почетных убийств для получения ранга {}.", playername, killsToNextRank, nextPvPRank - HORDE_PVP_RANK_OFFSET);
                return true;
            }
        }
    }
