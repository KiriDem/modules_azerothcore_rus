...
('transmog', 0, 'Syntax: .transmog <on/off>\nПозволяет видеть предметы с измененным внешним видом и NPC-трансмогрификатора.'),
('transmog add', 1, 'Syntax: .transmog add $player $item\nДобавляет предмет в коллекцию внешнего вида игрока.'),
('transmog sync', 0, 'Syntax: .transmog sync\nСинхронизирует внешний вид предметов из аддона для трансмогрификации с сервером.'),
('transmog add set', 1, 'Syntax: .transmog add set $player $itemSet\nДобавляет предметы из комплекта в коллекцию внешнего вида игрока.'),
('transmog portable', 1, 'Syntax: .transmog portable \nПризывает Эфирного ткача искажений — переносную версию NPC для трансмогрификации.'),
('transmog interface', 1, 'Syntax: .transmog interface <on/off>\nАктивирует интерфейс торговца для трансмогрификации, позволяющий предварительно просматривать внешний вид.\nОТКАЗ ОТ ОТВЕТСТВЕННОСТИ: Если у вас слишком много вызовов, некоторые из них не будут отображаться из-за ограничений клиента. В этом случае отключите эту опцию.'),
