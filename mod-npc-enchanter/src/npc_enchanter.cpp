...
enum Enchants
{
    ...
    ENCHANT_WEP_Мастер битвы = 2675,
    ENCHANT_WEP_BLACK_MAGIC = 3790,
    ENCHANT_WEP_Ледокол = 3239,
    ENCHANT_WEP_Живительная защита = 3241,
    ENCHANT_WEP_MIGHTY_SPELL_POWER = 3834, // One-hand
    ENCHANT_WEP_Палач = 3225,
    ...
    ENCHANT_2WEP_Мангуст = 2673,

    ENCHANT_SHIELD_DEFENSE = 1952,
    ENCHANT_SHIELD_к интелекту = 1128,
    ...
    ENCHANT_GLOVES_к экспертизе = 3231,
    ...
    ENCHANT_BRACERS_к экспертизе = 3231,
    ENCHANT_BRACERS_GREATER_STATS = 2661,
    ENCHANT_BRACERS_к интелекту = 1119,
...
public:
    ...
    void OnPlayerLogin(Player* player)
    {
        ...
        if (EnchanterAnnounceModule)
            ChatHandler(player->GetSession()).SendSysMessage("На этом сервере работает модуль |cff4CFF00EnchanterNPC |r.");
    }
};
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        AddGossipItemFor(player, 1, "|TInterface/ICONS/Inv_mace_116:24:24:-18|t[Зачаровать основное оружие]", GOSSIP_SENDER_MAIN, 1);
        if (player->HasSpell(674))
            AddGossipItemFor(player, 1, "|TInterface/ICONS/Inv_mace_116:24:24:-18|t[Зачаровать дополнительное оружие]", GOSSIP_SENDER_MAIN, 13);

        AddGossipItemFor(player, 1, "|TInterface/ICONS/Inv_axe_113:24:24:-18|t[Наложить чары на двуручное оружие]", GOSSIP_SENDER_MAIN, 2);
        AddGossipItemFor(player, 1, "|TInterface/ICONS/Inv_shield_71:24:24:-18|t[Чары для щита]", GOSSIP_SENDER_MAIN, 3);
        AddGossipItemFor(player, 1, "|TInterface/ICONS/inv_helmet_29:24:24:-18|t[Чары для шлема]", GOSSIP_SENDER_MAIN, 4);
        AddGossipItemFor(player, 1, "|TInterface/ICONS/inv_shoulder_23:24:24:-18|t[Чары для наплечников]", GOSSIP_SENDER_MAIN, 5);
        AddGossipItemFor(player, 1, "|TInterface/ICONS/Inv_misc_cape_18:24:24:-18|t[Чары для плаща]", GOSSIP_SENDER_MAIN, 6);
        AddGossipItemFor(player, 1, "|TInterface/ICONS/inv_chest_cloth_04:24:24:-18|t[Чары для брони]", GOSSIP_SENDER_MAIN, 7);
        AddGossipItemFor(player, 1, "|TInterface/ICONS/inv_bracer_14:24:24:-18|t[Чары для наручей]", GOSSIP_SENDER_MAIN, 8);
        AddGossipItemFor(player, 1, "|TInterface/ICONS/inv_gauntlets_06:24:24:-18|t[Зачаровать перчатки]", GOSSIP_SENDER_MAIN, 9);
        AddGossipItemFor(player, 1, "|TInterface/ICONS/inv_pants_11:24:24:-18|t[Чары для поножей]", GOSSIP_SENDER_MAIN, 10);
        AddGossipItemFor(player, 1, "|TInterface/ICONS/inv_boots_05:24:24:-18|t[Чары для сапог]", GOSSIP_SENDER_MAIN, 11);

        if (player->HasSkill(SKILL_ENCHANTING) && player->GetSkillValue(SKILL_ENCHANTING) == 450)
            AddGossipItemFor(player, 1, "|TInterface/ICONS/Inv_jewelry_ring_85:24:24:-18|t[Зачаровать кольца]", GOSSIP_SENDER_MAIN, 12);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
    {
        ...
        case 1: // Enchant Main Hand Weapon
            if (player->HasSkill(SKILL_ENCHANTING) && player->GetSkillValue(SKILL_ENCHANTING) == 450)
            {
                AddGossipItemFor(player, 1, "Защитный клинок", GOSSIP_SENDER_MAIN, 102);
                AddGossipItemFor(player, 1, "Вытягивание крови", GOSSIP_SENDER_MAIN, 103);
            }
            AddGossipItemFor(player, 1, "26 к ловкости", GOSSIP_SENDER_MAIN, 100);
            AddGossipItemFor(player, 1, "45 к духу", GOSSIP_SENDER_MAIN, 101);
            AddGossipItemFor(player, 1, "Берсерк", GOSSIP_SENDER_MAIN, 104);
            AddGossipItemFor(player, 1, "25 к рейтингу меткости + 25 к критическому удару", GOSSIP_SENDER_MAIN, 105);
            AddGossipItemFor(player, 1, "Черная магия", GOSSIP_SENDER_MAIN, 106);
            AddGossipItemFor(player, 1, "Мастер битвы", GOSSIP_SENDER_MAIN, 107);
            AddGossipItemFor(player, 1, "Ледокол", GOSSIP_SENDER_MAIN, 108);
            AddGossipItemFor(player, 1, "Живительная защита", GOSSIP_SENDER_MAIN, 109);
            AddGossipItemFor(player, 1, "50 к выносливости", GOSSIP_SENDER_MAIN, 110);
            AddGossipItemFor(player, 1, "65 к силе атаки", GOSSIP_SENDER_MAIN, 111);
            AddGossipItemFor(player, 1, "63 к силе заклинаний", GOSSIP_SENDER_MAIN, 112);
            AddGossipItemFor(player, 1, "Мангуст", GOSSIP_SENDER_MAIN, 113);
            AddGossipItemFor(player, 1, "Палач", GOSSIP_SENDER_MAIN, 114);
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "Назад", GOSSIP_SENDER_MAIN, 300);
        ...
        case 2: // Enchant 2H Weapon
            ...
            if (!item)
            {
                creature->Whisper("Для этого зачарования необходимо, чтобы было экипировано двуручное оружие.", LANG_UNIVERSAL, player);
            ...
            if (item->GetTemplate()->InventoryType == INVTYPE_2HWEAPON)
            {
                ...
                AddGossipItemFor(player, 1, "Мангуст", GOSSIP_SENDER_MAIN, 113);
                AddGossipItemFor(player, 1, "Палач", GOSSIP_SENDER_MAIN, 114);
                AddGossipItemFor(player, 1, "81 к силе заклинаний", GOSSIP_SENDER_MAIN, 115);
                AddGossipItemFor(player, 1, "35 к ловкости", GOSSIP_SENDER_MAIN, 116);
                AddGossipItemFor(player, 1, "110 к силе атаки", GOSSIP_SENDER_MAIN, 117);
                AddGossipItemFor(player, GOSSIP_ICON_TALK, "Назад", GOSSIP_SENDER_MAIN, 300);
            }
            else
            {
        ...
        case 3: // Enchant Shield
            ...
            if (!item)
            {
                creature->Whisper("Для этого зачарования необходимо, чтобы был экипирован щит.", LANG_UNIVERSAL, player);
                player->PlayerTalkClass->SendCloseGossip();
                return false;
            }
            if (item->GetTemplate()->InventoryType == INVTYPE_SHIELD)
            {
                AddGossipItemFor(player, 1, "20 к защите", GOSSIP_SENDER_MAIN, 118);
                AddGossipItemFor(player, 1, "25 к интелекту", GOSSIP_SENDER_MAIN, 119);
                AddGossipItemFor(player, 1, "12 к устойчивости", GOSSIP_SENDER_MAIN, 120);
                AddGossipItemFor(player, 1, "36 к блоку", GOSSIP_SENDER_MAIN, 121);
                AddGossipItemFor(player, 1, "18 к выносливости", GOSSIP_SENDER_MAIN, 122);
                AddGossipItemFor(player, 1, "81 к блоку + 50% к обезоруживанию", GOSSIP_SENDER_MAIN, 123);
                AddGossipItemFor(player, GOSSIP_ICON_TALK, "Назад", GOSSIP_SENDER_MAIN, 300);
            }
            else
            {
        ...
        case 4: // Enchant Head
            AddGossipItemFor(player, 1, "30 к силе заклинаний + 10 к мане", GOSSIP_SENDER_MAIN, 124);
            AddGossipItemFor(player, 1, "30 к силе заклинаний + 20 к критическому урону", GOSSIP_SENDER_MAIN, 125);
            AddGossipItemFor(player, 1, "29 к силе заклинаний + 20 к устойчивости", GOSSIP_SENDER_MAIN, 126);
            AddGossipItemFor(player, 1, "30 к выносливости + 25 к устойчивости", GOSSIP_SENDER_MAIN, 127);
            AddGossipItemFor(player, 1, "37 к выносливости + 20 к защите", GOSSIP_SENDER_MAIN, 128);
            AddGossipItemFor(player, 1, "50 к силе атаки + 20 к критическому урону", GOSSIP_SENDER_MAIN, 129);
            AddGossipItemFor(player, 1, "50 к силе атаки + 20 к устойчивости", GOSSIP_SENDER_MAIN, 130);
            AddGossipItemFor(player, 1, "Аркан затмения Луны", GOSSIP_SENDER_MAIN, 131);
            AddGossipItemFor(player, 1, "Арканум души пламени", GOSSIP_SENDER_MAIN, 132);
            AddGossipItemFor(player, 1, "Арканум ускользающей тени", GOSSIP_SENDER_MAIN, 133);
            AddGossipItemFor(player, 1, "Арканум ледяной души", GOSSIP_SENDER_MAIN, 134);
            AddGossipItemFor(player, 1, "Арканум защиты от яда", GOSSIP_SENDER_MAIN, 135);
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "Назад", GOSSIP_SENDER_MAIN, 300);
        ...
        case 5: // Enchant Shoulders
            ...
            {
                AddGossipItemFor(player, 1, "120 к силе атаки + 15 к критическому урону", GOSSIP_SENDER_MAIN, 136);
                AddGossipItemFor(player, 1, "70 к силе заклинаний + 8 к мане", GOSSIP_SENDER_MAIN, 137);
                AddGossipItemFor(player, 1, "60 к уклонению + 15 к защите", GOSSIP_SENDER_MAIN, 138);
                AddGossipItemFor(player, 1, "70 к силе заклинаний + 15 к критическому урону", GOSSIP_SENDER_MAIN, 139);
            }
            AddGossipItemFor(player, 1, "40 к силе атаки + 15 к критическому урону", GOSSIP_SENDER_MAIN, 140);
            AddGossipItemFor(player, 1, "24 к силе заклинаний + 8 к мане", GOSSIP_SENDER_MAIN, 141);
            AddGossipItemFor(player, 1, "30 к выносливости + 15 к устойчивости", GOSSIP_SENDER_MAIN, 142);
            AddGossipItemFor(player, 1, "20 к уклонению + 15 к защите", GOSSIP_SENDER_MAIN, 143);
            AddGossipItemFor(player, 1, "24 к силе заклинаний + 15 к критическому урону", GOSSIP_SENDER_MAIN, 144);
            AddGossipItemFor(player, 1, "23 к силе заклинаний + 15 к устойчивости", GOSSIP_SENDER_MAIN, 145);
            AddGossipItemFor(player, 1, "40 к силе атаки + 15 к устойчивости", GOSSIP_SENDER_MAIN, 146);
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "Назад", GOSSIP_SENDER_MAIN, 300);
        ...
        case 6: // Enchant Cloak
            ...
            {
                AddGossipItemFor(player, 1, "Вышивка Темное свечение", GOSSIP_SENDER_MAIN, 149);
                AddGossipItemFor(player, 1, "Вышивка легким плетением", GOSSIP_SENDER_MAIN, 150);
                AddGossipItemFor(player, 1, "Вышивка на гарде меча", GOSSIP_SENDER_MAIN, 151);
            }
            if (player->HasSkill(SKILL_ENGINEERING) && player->GetSkillValue(SKILL_ENGINEERING) == 450)
            {
                AddGossipItemFor(player, 1, "Парашют", GOSSIP_SENDER_MAIN, 147);
            }
            AddGossipItemFor(player, 1, "Теневой доспех", GOSSIP_SENDER_MAIN, 148);
            AddGossipItemFor(player, 1, "10 Spirit + 2% к уменьшению угрозы", GOSSIP_SENDER_MAIN, 152);
            AddGossipItemFor(player, 1, "16 к защите", GOSSIP_SENDER_MAIN, 153);
            AddGossipItemFor(player, 1, "35 к проникновению заклинаний", GOSSIP_SENDER_MAIN, 154);
            AddGossipItemFor(player, 1, "225 к броне", GOSSIP_SENDER_MAIN, 155);
            AddGossipItemFor(player, 1, "22 к ловкости", GOSSIP_SENDER_MAIN, 156);
            AddGossipItemFor(player, 1, "23 к спешке", GOSSIP_SENDER_MAIN, 157);
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "Назад", GOSSIP_SENDER_MAIN, 300);
        ...
        case 7: //Enchant chest
            AddGossipItemFor(player, 1, "+10 Все статы", GOSSIP_SENDER_MAIN, 158);
            AddGossipItemFor(player, 1, "225 к здоровью", GOSSIP_SENDER_MAIN, 159);
            AddGossipItemFor(player, 1, "10 к мане", GOSSIP_SENDER_MAIN, 160);
            AddGossipItemFor(player, 1, "20 к устойчивости", GOSSIP_SENDER_MAIN, 161);
            AddGossipItemFor(player, 1, "22 к защите", GOSSIP_SENDER_MAIN, 162);
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "Назад", GOSSIP_SENDER_MAIN, 300);
        ...
        case 8: //Enchant Bracers
            AddGossipItemFor(player, 1, "40 к выносливости", GOSSIP_SENDER_MAIN, 163);
            AddGossipItemFor(player, 1, "30 к силе заклинаний", GOSSIP_SENDER_MAIN, 164);
            AddGossipItemFor(player, 1, "50 к силе атаки", GOSSIP_SENDER_MAIN, 165);
            AddGossipItemFor(player, 1, "18 к духу", GOSSIP_SENDER_MAIN, 166);
            AddGossipItemFor(player, 1, "15 к экспертизе", GOSSIP_SENDER_MAIN, 167);
            AddGossipItemFor(player, 1, "+6 все статы", GOSSIP_SENDER_MAIN, 168);
            AddGossipItemFor(player, 1, "16 к интелекту", GOSSIP_SENDER_MAIN, 169);
            if (player->HasSkill(SKILL_LEATHERWORKING) && player->GetSkillValue(SKILL_LEATHERWORKING) == 450)
            {
                AddGossipItemFor(player, 1, "Меховая подкладка - Сопротивление магии", GOSSIP_SENDER_MAIN, 170);
                AddGossipItemFor(player, 1, "Меховая подкладка - Сопротивление огню", GOSSIP_SENDER_MAIN, 171);
                AddGossipItemFor(player, 1, "Меховая подкладка - Сопротивление холоду", GOSSIP_SENDER_MAIN, 172);
                AddGossipItemFor(player, 1, "Меховая подкладка - Сопротивление силам природы", GOSSIP_SENDER_MAIN, 173);
                AddGossipItemFor(player, 1, "Меховая подкладка - К сопротивлению тени", GOSSIP_SENDER_MAIN, 174);
                AddGossipItemFor(player, 1, "Меховая подкладка - к силе атаки", GOSSIP_SENDER_MAIN, 175);
                AddGossipItemFor(player, 1, "Меховая подкладка - к выносливости", GOSSIP_SENDER_MAIN, 176);
                AddGossipItemFor(player, 1, "Меховая подкладка - к силе заклинаний", GOSSIP_SENDER_MAIN, 177);
            }
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "Назад", GOSSIP_SENDER_MAIN, 300);
        ...
        case 9: //Enchant Gloves
            ...
            {
                AddGossipItemFor(player, 1, "Гиперскоростные ускорители", GOSSIP_SENDER_MAIN, 200);
            }
            AddGossipItemFor(player, 1, "16 к критическому удару", GOSSIP_SENDER_MAIN, 178);
            AddGossipItemFor(player, 1, "2% к угрозе + 10 к парированию", GOSSIP_SENDER_MAIN, 179);
            AddGossipItemFor(player, 1, "44 к силе атаки", GOSSIP_SENDER_MAIN, 180);
            AddGossipItemFor(player, 1, "20 к ловкости", GOSSIP_SENDER_MAIN, 181);
            AddGossipItemFor(player, 1, "20 к рейтингу меткости", GOSSIP_SENDER_MAIN, 182);
            AddGossipItemFor(player, 1, "15 к экспертизе", GOSSIP_SENDER_MAIN, 183);
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "Назад", GOSSIP_SENDER_MAIN, 300);
        ...
        case 10: //Enchant legs
            AddGossipItemFor(player, 1, "40 к устойчивости + 28 к выносливости", GOSSIP_SENDER_MAIN, 184);
            AddGossipItemFor(player, 1, "55 к выносливости + 22 к ловкости", GOSSIP_SENDER_MAIN, 185);
            AddGossipItemFor(player, 1, "75 к силе атаки + 22 к критическому удару", GOSSIP_SENDER_MAIN, 186);
            AddGossipItemFor(player, 1, "50 к силе заклинаний + 22 к духу", GOSSIP_SENDER_MAIN, 187);
            AddGossipItemFor(player, 1, "50 к силе заклинаний + 30 к выносливости", GOSSIP_SENDER_MAIN, 188);
            AddGossipItemFor(player, 1, "72 к выносливости + 35 к ловкости", GOSSIP_SENDER_MAIN, 189);
            AddGossipItemFor(player, 1, "100 к силе атаки + 36 к критическому удару", GOSSIP_SENDER_MAIN, 190);
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "Назад", GOSSIP_SENDER_MAIN, 300);
        ...
        case 11: //Enchant Boots
            AddGossipItemFor(player, 1, "32 к силе атаки", GOSSIP_SENDER_MAIN, 191);
            AddGossipItemFor(player, 1, "15 к выносливости + Minor Speed Increase", GOSSIP_SENDER_MAIN, 192);
            AddGossipItemFor(player, 1, "16 к ловкости", GOSSIP_SENDER_MAIN, 193);
            AddGossipItemFor(player, 1, "18 к духу", GOSSIP_SENDER_MAIN, 194);
            AddGossipItemFor(player, 1, "Восстановить 7 здоровья + Мана 5", GOSSIP_SENDER_MAIN, 195);
            AddGossipItemFor(player, 1, "12 к рейтингу меткости + 12 к критическому удару", GOSSIP_SENDER_MAIN, 196);
            AddGossipItemFor(player, 1, "22 к выносливости", GOSSIP_SENDER_MAIN, 197);
            if (player->HasSkill(SKILL_ENGINEERING) && player->GetSkillValue(SKILL_ENGINEERING) == 450)
            {
                AddGossipItemFor(player, 1, "Нитро ботинки", GOSSIP_SENDER_MAIN, 198);
                AddGossipItemFor(player, 1, "Ручная пироракета", GOSSIP_SENDER_MAIN, 199);
                AddGossipItemFor(player, 1, "Сетчатая броневая лента", GOSSIP_SENDER_MAIN, 201);
            }
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "Назад", GOSSIP_SENDER_MAIN, 300);
        ...
        case 12: //Enchant rings
            AddGossipItemFor(player, 1, "40 к силе атаки", GOSSIP_SENDER_MAIN, 202);
            AddGossipItemFor(player, 1, "23 к силе заклинаний", GOSSIP_SENDER_MAIN, 203);
            AddGossipItemFor(player, 1, "30 к выносливости", GOSSIP_SENDER_MAIN, 204);
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "Назад", GOSSIP_SENDER_MAIN, 300);
        ...
        case 13: // Enchant 1H Weapon
            ...
            if (!item)
            {
                creature->Whisper("Для этого зачарования требуется, чтобы оружие было экипировано в левой руке.", LANG_UNIVERSAL, player);
                player->PlayerTalkClass->SendCloseGossip();
                return false;
            }
            if (item->GetTemplate()->InventoryType == INVTYPE_WEAPON)
            {
                if (player->HasSkill(SKILL_ENCHANTING) && player->GetSkillValue(SKILL_ENCHANTING) == 450)
                {
                    AddGossipItemFor(player, 1, "Защитный клинок", GOSSIP_SENDER_MAIN, 207);
                    AddGossipItemFor(player, 1, "Вытягивание крови", GOSSIP_SENDER_MAIN, 208);
                }
                AddGossipItemFor(player, 1, "26 к ловкости", GOSSIP_SENDER_MAIN, 205);
                AddGossipItemFor(player, 1, "45 к духу", GOSSIP_SENDER_MAIN, 206);
                AddGossipItemFor(player, 1, "Берсерк", GOSSIP_SENDER_MAIN, 209);
                AddGossipItemFor(player, 1, "25 к меткости и +25 к критическому удару", GOSSIP_SENDER_MAIN, 210);
                AddGossipItemFor(player, 1, "Черная магия", GOSSIP_SENDER_MAIN, 211);
                AddGossipItemFor(player, 1, "Мастер битвы", GOSSIP_SENDER_MAIN, 212);
                AddGossipItemFor(player, 1, "Ледокол", GOSSIP_SENDER_MAIN, 213);
                AddGossipItemFor(player, 1, "Живительная защита", GOSSIP_SENDER_MAIN, 214);
                AddGossipItemFor(player, 1, "50 к выносливости", GOSSIP_SENDER_MAIN, 215);
                AddGossipItemFor(player, 1, "65 к силе атаки", GOSSIP_SENDER_MAIN, 216);
                AddGossipItemFor(player, 1, "63 к силе заклинаний", GOSSIP_SENDER_MAIN, 217);
                AddGossipItemFor(player, 1, "Мангуст", GOSSIP_SENDER_MAIN, 218);
                AddGossipItemFor(player, 1, "Палач", GOSSIP_SENDER_MAIN, 219);
                AddGossipItemFor(player, GOSSIP_ICON_TALK, "Назад", GOSSIP_SENDER_MAIN, 300);
            }
            else
            {
        ...
        case 104:
            Enchant(player, creature, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND), ENCHANT_WEP_Берсерк);
            break;
        ...
        case 107:
            Enchant(player, creature, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND), ENCHANT_WEP_Мастер битвы);
            break;

        case 108:
            Enchant(player, creature, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND), ENCHANT_WEP_Ледокол);
            break;

        case 109:
            Enchant(player, creature, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND), ENCHANT_WEP_Живительная защита);
            break;
        ...
        case 113:
            Enchant(player, creature, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND), ENCHANT_2WEP_Мангуст);
            break;

        case 114:
            Enchant(player, creature, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND), ENCHANT_WEP_Палач);
            break;
        ...
        case 119:
            Enchant(player, creature, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND), ENCHANT_SHIELD_к интелекту);
            break;
        ...
        case 167:
            Enchant(player, creature, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_WRISTS), ENCHANT_BRACERS_к экспертизе);
            break;
        ...
        case 169:
            Enchant(player, creature, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_WRISTS), ENCHANT_BRACERS_к интелекту);
            break;
        ...
        case 183:
            Enchant(player, creature, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_HANDS), ENCHANT_GLOVES_к экспертизе);
            break;
        ...
        case 209:
            Enchant(player, creature, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND), ENCHANT_WEP_Берсерк);
            break;
        ...
        case 212:
            Enchant(player, creature, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND), ENCHANT_WEP_Мастер битвы);
            break;

        case 213:
            Enchant(player, creature, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND), ENCHANT_WEP_Ледокол);
            break;

        case 214:
            Enchant(player, creature, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND), ENCHANT_WEP_Живительная защита);
            break;
        ...
        case 218:
            Enchant(player, creature, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND), ENCHANT_2WEP_Мангуст);
            break;

        case 219:
            Enchant(player, creature, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND), ENCHANT_WEP_Палач);
            break;
        ...
        case 300:
        {
            ...
            if (player->HasSpell(674))
            {
                AddGossipItemFor(player, 1, "|TInterface/ICONS/Inv_mace_116:24:24:-18|t[Зачаровать оружие в левой руке]", GOSSIP_SENDER_MAIN, 13);
            }
            AddGossipItemFor(player, 1, "|TInterface/ICONS/Inv_axe_113:24:24:-18|t[Наложить чары на двуручное оружие]", GOSSIP_SENDER_MAIN, 2);
            AddGossipItemFor(player, 1, "|TInterface/ICONS/Inv_shield_71:24:24:-18|t[Чары для щита]", GOSSIP_SENDER_MAIN, 3);
            AddGossipItemFor(player, 1, "|TInterface/ICONS/inv_helmet_29:24:24:-18|t[Чары для шлема]", GOSSIP_SENDER_MAIN, 4);
    ...
    void Enchant(Player* player, Creature* creature, Item* item, uint32 enchantid)
    {
        if (!item)
        {
            ...
            creature->Whisper("Пожалуйста, наденьте предмет, который вы хотите зачаровать!", LANG_UNIVERSAL, player);
            player->PlayerTalkClass->SendCloseGossip();
            return;
        }
        ...
        if (!enchantid)
        {
        	ChatHandler(player->GetSession()).SendNotification("В коде произошел сбой. Информация об этом передана разработчикам для проверки, приносим извинения за неудобства.");
            player->PlayerTalkClass->SendCloseGossip();
            creature->HandleEmoteCommand(EMOTE_ONESHOT_LAUGH);
            return;
        }
        ...
        if (roll > 0 && roll < 33)
        {
            ChatHandler(player->GetSession()).SendNotification("|cff00ff00Костлявый палец Борегара потрескивает от энергии, когда он прикасается. |cffDA70D6%s|cff00ff00!", item->GetTemplate()->Name1.c_str());
            // creature->HandleEmoteCommand(EMOTE_ONESHOT_POINT);
        }
        else if (roll > 33 && roll < 75)
        {
            ChatHandler(player->GetSession()).SendNotification("|cff00ff00Борегард поднимает |cffDA70D6%s |cff00ff00в воздух и произносит странное заклинание!", item->GetTemplate()->Name1.c_str());
            // creature->HandleEmoteCommand(EMOTE_ONESHOT_CHEER);
        }
        else
        {
            ChatHandler(player->GetSession()).SendNotification("|cff00ff00Борегар глубоко сосредоточен, взмахивая волшебной палочкой над |cffDA70D6%s|cff00ff00!", item->GetTemplate()->Name1.c_str());
