...
public:
    ...
    void OnLogin(Player *player) {
        ...
        if (ObjscaleAnnounceModule) {
            ChatHandler(player->GetSession()).SendSysMessage("Этот сервер использует модуль |cff4CFF00Objscale |r");
        }
    }
};
...
public:
    ...
    static bool HandleSetCommand(ChatHandler *handler, float scale) {
        ...
        if (scale > 0) {
            ...
            me->GetSession()->SendAreaTriggerMessage("Масштаб существа был обновлен до %f", scale);
            return true;
        } else {
            me->GetSession()->SendAreaTriggerMessage("масштаб должен быть больше 0.");
            return false;
        }
    }
    ...
    static bool HandleReSetCommand(ChatHandler *handler) {
        ...
        float scale = creature->GetObjectScale();
        creature->SetObjectScale(scale);
        if (!creature->IsPet())
            creature->SaveToDB();

        me->GetSession()->SendAreaTriggerMessage("Масштаб существа был сброшен до %f", scale);
        return true;
    ...
    static bool HandleBaseCommand(ChatHandler *handler, char const *args) {
        ...
        me->GetSession()->SendAreaTriggerMessage("Используйте комманду set или reset");
        return true;
    }
};
...
public:
    ...
    static bool HandleSetCommand(ChatHandler *handler, GameObjectSpawnId guidLow, float scale) {
        ...
        if (scale > 0.0f) {
            ...
            me->GetSession()->SendAreaTriggerMessage("Масштаб объекта был обновлен до %f", scale);
            return true;
        } else {
            me->GetSession()->SendAreaTriggerMessage("масштаб должен быть больше 0.");
            return false;
    ...
    static bool HandleReSetCommand(ChatHandler *handler, GameObjectSpawnId guidLow) {
        ...
        me->GetSession()->SendAreaTriggerMessage("Масштаб объекта был сброшен до %f", scale);
        return true;
    }
