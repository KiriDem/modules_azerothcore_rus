...
local function PrintHelp()
    PrintMsg("Команды: /qr (синхронизация + отладка), /qr on|off (включение/выключение), /qr zone on|off (показать/скрыть зону), /qr tracked on|off (ограничить отслеживаемыми заданиями), /qr scan (диагностика журнала заданий), /qr help.")
end
...
SlashCmdList["QUESTRADAR"] = function(msg)
    ...
    if cmd == "zone" then
        ...
        if sub == "on" or sub == "off" then
            ...
            PrintMsg("Использование: /qr zone on|off (сейчас: %s).", QR.db.showArea and "on" or "off")
        end
        return
    end
    ...
    if cmd == "tracked" then
        ...
        if sub == "on" or sub == "off" then
            ...
            PrintMsg("Ограничение для отслеживаемых заданий: %s.", QR.db.onlyTracked and "yes" or "no")
            if QR.RefreshIcons then
                QR.RefreshIcons()
            end
        else
            PrintMsg("Использование: /qr отслеживание on|off (в настоящее время %s).", QR.db.onlyTracked and "on" or "off")
        end
        return
    end
    ...
    if cmd == "scan" then
        ...
        PrintMsg("Сканирование журнала заданий (%d entries):", numEntries)
    ...
    if cmd ~= "" then
        PrintMsg("Неизвестная команда: %s", cmd)
        PrintHelp()
        return
    end
    ...
    RequestSync()
    PrintMsg("Запрошена синхронизация — %d целей на карте %s (игрок: %s, %s). Модуль %s, зона %s, только отслеживаемые: %s.",
