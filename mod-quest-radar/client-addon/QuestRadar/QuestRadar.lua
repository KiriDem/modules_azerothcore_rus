...
driver:SetScript("OnEvent", function(self, event)
    ...
    if event == "PLAYER_LOGIN" or event == "PLAYER_ENTERING_WORLD" then
        ...
        if not GetAstrolabe() then
            Print("|cffff4040!Астролябия не найдена|r - Interface\\AddOns\\!Астролябия должна быть в наличии и активна.")
        end
    end
    Invalidate()
end)
...
local function Status()
    ...
    Print("карта: континент=" .. tostring(continent) .. " zone=" .. tostring(zone)
        .. " (" .. tostring(GetZoneText()) .. ")")
    Print("запрос=" .. lastCounts.quests
    ...
    if lastCounts.pois == 0 then
        Print("0 POI: Если на карте мира также не отображаются пронумерованные метки, значит, таблица "
            .. "quest_poi для этого мира пуста на стороне сервера.")
    end
end
...
SlashCmdList["QUESTRADAR"] = function(msg)
    ...
    elseif cmd == "completed" then
        ...
        Invalidate()
        Print("Выполненные запросы: " .. (DB.showCompleted and "affichees" or "masquees"))
    elseif cmd == "tracked" then
        DB.onlyTracked = not DB.onlyTracked
        Invalidate()
        Print(DB.onlyTracked
            and "Отображаются только отслеживаемые задания."
            or "Отображаются все задания в этой зоне.")
    elseif cmd == "module" then
        DB.useModule = not DB.useModule
        Invalidate()
        Print("серверный модуль: " .. (DB.useModule and "использовать, если имеется" or "игнорировать"))
    elseif cmd == "edge" or cmd == "arrows" then
        DB.showOffscreen = not DB.showOffscreen
        Print("недостижимые цели (расположенные у края): "
    ...
    elseif cmd == "scale" then
        ...
        if value and value >= 0.5 and value <= 3 then
            ...
            ApplyScale()
            Invalidate()
            Print("Масштаб значков = " .. value)
        else
            Print("Шкала: значение от 0,5 до 3")
        end
    else
