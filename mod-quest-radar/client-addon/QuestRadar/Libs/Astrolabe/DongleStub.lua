...
if not g.DongleStub or g.DongleStub:IsNewerVersion(major, minor) then
	local lib = setmetatable({}, {
		__call = function(t,k) 
			if type(t.versions) == "table" and t.versions[k] then 
				...
				error("Не удается найти библиотеку с именем '"..tostring(k).."'", 2)
			end
		end
	})
	...
	function lib:Register(newInstance, activate, deactivate)
		assert(type(newInstance.GetVersion) == "function",
			"Попытка зарегистрировать в DongleStub библиотеку, не имеющую метода «GetVersion».")

		local major,minor = newInstance:GetVersion()
		assert(type(major) == "string",
			"Попытка зарегистрировать в DongleStub библиотеку, у которой отсутствует корректная старшая версия (major version).")
		assert(type(minor) == "number",
			"Попытка зарегистрировать библиотеку в DongleStub, у которой отсутствует корректная дополнительная версия.")

		-- Generate a log of all library registrations
		if not self.log then self.log = {} end
		table.insert(self.log, string.format("Регистрация: %s, %s", major, minor))
		...
		if not versionData then
			...
			if type(activate) == "function" then
				table.insert(self.log, string.format("Активировать: %s, %s", major, minor))
		...
		if type(oldDeactivate) == "function" then
			...
			table.insert(self.log, string.format("Деактивировать: %s, %s", major, minor))
