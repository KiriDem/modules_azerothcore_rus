...
-- FAVOURITE LIST
local FavFrame = GOMove:CreateFrame("Список_избранного", 200, 280, GOMove.FavL, true)
...
-- SELECTION LIST
local SelFrame = GOMove:CreateFrame("Список_выбора", 250, 280, GOMove.SelL, true)
...
local NORTH = GOMove:CreateButton(MainFrame, "C", 50, 25, 0, -25)
function NORTH:OnClick()
    GOMove:Move("NORTH", NEWS:GetNumber())
end
local EAST = GOMove:CreateButton(MainFrame, "В", 50, 25, 50, -50)
function EAST:OnClick()
    GOMove:Move("EAST", NEWS:GetNumber())
end
local SOUTH = GOMove:CreateButton(MainFrame, "Ю", 50, 25, 0, -75)
function SOUTH:OnClick()
    GOMove:Move("SOUTH", NEWS:GetNumber())
end
local WEST = GOMove:CreateButton(MainFrame, "З", 50, 25, -50, -50)
function WEST:OnClick()
    GOMove:Move("WEST", NEWS:GetNumber())
end

local NORTHEAST = GOMove:CreateButton(MainFrame, "СВ", 40, 20, 45, -30)
function NORTHEAST:OnClick()
    GOMove:Move("NORTHEAST", NEWS:GetNumber())
end
local NORTHWEST = GOMove:CreateButton(MainFrame, "СЗ", 40, 20, -45, -30)
function NORTHWEST:OnClick()
    GOMove:Move("NORTHWEST", NEWS:GetNumber())
end
local SOUTHEAST = GOMove:CreateButton(MainFrame, "ЮВ", 40, 20, 45, -75)
function SOUTHEAST:OnClick()
    GOMove:Move("SOUTHEAST", NEWS:GetNumber())
end
local SOUTHWEST = GOMove:CreateButton(MainFrame, "ЮЗ", 40, 20, -45, -75)
...
local UP = GOMove:CreateButton(MainFrame, "Вверх", 40, 25, 0, -130)
function UP:OnClick()
    GOMove:Move("UP", ROTHEI:GetNumber())
end
local DOWN = GOMove:CreateButton(MainFrame, "Вниз", 40, 25, 0, -180)
function DOWN:OnClick()
    GOMove:Move("DOWN", ROTHEI:GetNumber())
end

local RIGHT = GOMove:CreateButton(MainFrame, "Вправо", 40, 25, 45, -155)
function RIGHT:OnClick()
    GOMove:Move("RIGHT", ROTHEI:GetNumber())
end
local LEFT = GOMove:CreateButton(MainFrame, "Влево", 40, 25, -45, -155)
function LEFT:OnClick()
    GOMove:Move("LEFT", ROTHEI:GetNumber())
end

local RESPAWN = GOMove:CreateButton(MainFrame, "Возродить", 65, 25, -35, -237.5)
function RESPAWN:OnClick()
    GOMove:Move("RESPAWN")
end
local FLOOR = GOMove:CreateButton(MainFrame, "Пол", 65, 25, 35, -237.5)
function FLOOR:OnClick()
    GOMove:Move("FLOOR")
end
local SELECTNEAR = GOMove:CreateButton(MainFrame, "Цель", 50, 25, 55, -210)
function SELECTNEAR:OnClick()
    GOMove:Move("SELECTNEAR")
end
local FACE = GOMove:CreateButton(MainFrame, "Разв.", 50, 25, 0, -210)
function FACE:OnClick()
    GOMove:Move("FACE")
end
local DELETE = GOMove:CreateButton(MainFrame, "Удалить", 50, 25, -55, -210)
function DELETE:OnClick()
    GOMove:Move("DELETE")
end

local GROUND = GOMove:CreateButton(MainFrame, "Земля", 70, 25, -40, -265)
function GROUND:OnClick()
    GOMove:Move("GROUND")
end
local GOTO = GOMove:CreateButton(MainFrame, "Идти к", 70, 25, 40, -265)
function GOTO:OnClick()
    GOMove:Move("GOTO")
end

local ENTRY = GOMove:CreateInput(MainFrame, "ENTRY", 65, 25, -30, -295, 10)
local SPAWN = GOMove:CreateButton(MainFrame, "Вызвать", 50, 25, 40, -295)
function SPAWN:OnClick()
    GOMove:Move("SPAWN", ENTRY:GetNumber())
end

local RADIUS = GOMove:CreateInput(MainFrame, "RADIUS", 40, 25, -55, -325, 4, 3)
local SELECTALLNEAR = GOMove:CreateButton(MainFrame, "Выбр. по радиусу", 110, 25, 25, -325)
...
local PHASE = GOMove:CreateButton(MainFrame, "Фаза", 50, 25, 40, -355)
function PHASE:OnClick()
    GOMove:Move("PHASE", MASK:GetNumber())
end

local FAVOURITES = GOMove:CreateButton(MainFrame, "Избранное", 80, 25, -40, -385)
...
local SELECTIONS = GOMove:CreateButton(MainFrame, "Подборка", 80, 25, 40, -385)
...
local SPELLSPAWN = GOMove:CreateButton(MainFrame, "Отправ.", 50, 25, 40, -415)
...
local SIZEPLUS = GOMove:CreateButton(MainFrame, "Разм. +", 50, 25, 55, -445)
function SIZEPLUS:OnClick()
    GOMove:Move("SIZEPLUS", SIZEMOD:GetNumber())
end

local SIZEMINUS = GOMove:CreateButton(MainFrame, "Разм. -", 50, 25, -55, -445)
