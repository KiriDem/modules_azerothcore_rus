    ...
    static bool GOMove_Command(ChatHandler* handler, char const* args)
    {
        ...
        if (ID < SPAWN) // no args
        {
            if (ID >= DELET && ID <= GOTO) // has target (needs retrieve gameobject)
            {
                ...
                if (!target)
                    ChatHandler(player->GetSession()).PSendSysMessage("Объект с GUID: %u не найден.", lowguid);
                else
                {
                    float x, y, z, o;
                    ...
                    case GOTO:
                    {
                        ...
                        {
                            ChatHandler(player->GetSession()).PSendSysMessage("Попробуйте снова после завершения полета.");
                            break;
                        }
                        else
                        {
            ...
            else
            {
                ...
                case SELECTNEAR:
                {
                    ...
                    if (!object)
                        ChatHandler(player->GetSession()).PSendSysMessage("Объекты не найдены");
                    else
                    {
                        GOMove::SendAdd(player, object->GetSpawnId());
                        session->SendAreaTriggerMessage("Выбранно %s", object->GetName().c_str());
                    }
                } break;
                }
            }
...
public:
    ...
    void OnLogin(Player* player)
    {
        ...
        {
            ChatHandler(player->GetSession()).SendSysMessage("На этом сервере запущен модуль |cff4CFF00GO-Move |r");
        }
    }
};
