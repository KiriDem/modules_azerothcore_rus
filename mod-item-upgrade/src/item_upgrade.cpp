...
bool ItemUpgrade::_AddPagedData(Player* player, const PagedData& pagedData, uint32 page) const
{
        ...
        || pagedData.type == PAGED_DATA_TYPE_WEAPON_SPEED_UPGRADE_PERCS || pagedData.type == PAGED_DATA_TYPE_WEAPON_SPEED_UPGRADE_PERC_INFO || pagedData.type == PAGED_DATA_TYPE_WEAPON_SPEED_UPGRADE_ITEMS_CHECK_INFO)
    {
        ...
        else if (pagedData.type == PAGED_DATA_TYPE_REQS)
        {
            ...
            oss << "УЛУЧШИТЬ " << StatTypeToString(upgradeStat->statType) << " [РАНГ " << upgradeStat->statRank << "]";
        ...
        else if (pagedData.type == PAGED_DATA_TYPE_UPGRADED_ITEMS_STATS)
        {
            ...
            if (weaponUpgrade != nullptr)
            {
                ...
                wuoss << "|cff056e3aУРОН ОТ ОРУЖИЯ УЛУЧШЕН " << FormatFloat(weaponUpgrade->statModPct) << "%|r";
            ...
            {
                std::ostringstream wuoss;
                wuoss << "|cff056e3aСКОРОСТЬ ОРУЖИЯ УЛУЧШЕНА " << FormatFloat(weaponSpeedUpgrade->statModPct) << "%|r";
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, wuoss.str(), GOSSIP_SENDER_MAIN + 2, GOSSIP_ACTION_INFO_DEF + page);
            }
        ...
        else if (pagedData.type == PAGED_DATA_TYPE_STAT_UPGRADE_BULK)
        {
            ...
            std::ostringstream oss;
            if (upgrades.empty())
                oss << "[УРОВЕНЬ ПРЕДМЕТА|cffb50505не|r повысится, улучшения применить нельзя]";
            else
            {
    ...
    {
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Всего улучшено предметов: " + Acore::ToString(pagedData.data.size()), GOSSIP_SENDER_MAIN + 2, GOSSIP_ACTION_INFO_DEF + page);
    ...
    else if (pagedData.type == PAGED_DATA_TYPE_ITEMS_FOR_PURGE)
    {
        ...
        if (proto != nullptr)
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Будет получено после очистки:", GOSSIP_SENDER_MAIN + 2, GOSSIP_ACTION_INFO_DEF + page);
        ...
        if (GetBoolConfig(CONFIG_ITEM_UPGRADE_REFUND_ALL_ON_PURGE))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|cff056e3aВЕРНЕМ ВСЕ ПРИ ОЧИСТКЕ|r", GOSSIP_SENDER_MAIN + 2, GOSSIP_ACTION_INFO_DEF + page);
    ...
    for (uint32 i = lowIndex; i <= highIndex; i++)
    {
        ...
        if (pagedData.type != PAGED_DATA_TYPE_ITEMS_FOR_PURGE)
            ...
            AddGossipItemFor(player, identifier->optionIcon, identifier->uiName, GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + identifier->id, "Вы уверены, что хотите удалить все улучшения? Это невозможно отменить!", 0, false);
    }

    if (pagedData.type == PAGED_DATA_TYPE_REQS)
        AddGossipItemFor(player, GOSSIP_ICON_TRAINER, (MeetsRequirement(player, pagedData.upgradeStat, item) ? "|cff056e3a[КУПИТЬ]|r" : "|cffb50505[КУПИТЬ]|r"), GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 1, "Вы уверенны, в своем решении?", 0, false);

    if (pagedData.type == PAGED_DATA_TYPE_WEAPON_UPGRADE_PERC_INFO)
        AddGossipItemFor(player, GOSSIP_ICON_TRAINER, (MeetsWeaponUpgradeRequirement(player) ? "|cff056e3a[УЛУЧШЕНИЕ]|r" : "|cffb50505[УЛУЧШЕНИЕ]|r"), GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 1, "Вы уверены, что хотите улучшить это оружие?", 0, false);

    if (pagedData.type == PAGED_DATA_TYPE_WEAPON_SPEED_UPGRADE_PERC_INFO)
        AddGossipItemFor(player, GOSSIP_ICON_TRAINER, (MeetsWeaponSpeedUpgradeRequirement(player) ? "|cff056e3a[УЛУЧШЕНИЕ]|r" : "|cffb50505[УЛУЧШЕНИЕ]|r"), GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 1, "Вы уверены, что хотите увеличить скорость этого оружия?", 0, false);
    ...
    if (!upgrades.empty())
    {
        ...
        AddGossipItemFor(player, GOSSIP_ICON_TRAINER, (MeetsRequirement(player, &reqs) ? "|cff056e3a[КУПИТЬ ВСЕ]|r" : "|cffb50505[КУПИТЬ ВСЕ]|r"), GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 2, "Вы уверенны, в своем решении?", 0, false);
    }

    if (pagedData.type == PAGED_DATA_TYPE_WEAPON_UPGRADE_ITEMS_CHECK_INFO)
        AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "|cffb50505УДАЛИТЬ УЛУЧШЕНИЕ|r", GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 1, "Вы уверены, что хотите удалить все улучшения? Это невозможно отменить!", 0, false);

    if (pagedData.type == PAGED_DATA_TYPE_WEAPON_SPEED_UPGRADE_ITEMS_CHECK_INFO)
        AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "|cffb50505УДАЛИТЬ УЛУЧШЕНИЕ СКОРОСТИ|r", GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 1, "Вы уверены, что хотите удалить все улучшения скорости? Это невозможно отменить!", 0, false);
...
void ItemUpgrade::NoPagedData(Player* player, const PagedData& pagedData) const
{
        ...
        || pagedData.type == PAGED_DATA_TYPE_WEAPON_UPGRADE_PERCS || pagedData.type == PAGED_DATA_TYPE_WEAPON_UPGRADE_PERC_INFO)
    {
        ...
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|cffb50505ПРЕДМЕТ НЕ МОЖЕТ БЫТЬ УЛУЧШЕН|r", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
    }
    else if (pagedData.type == PAGED_DATA_TYPE_ITEMS_FOR_PURGE)
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|cffb50505НЕЧЕГО ОЧИЩАТЬ|r", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
    else if (pagedData.type == PAGED_DATA_TYPE_WEAPON_SPEED_ITEMS)
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|cffb50505НА ЭТОЙ СТРАНИЦЕ НИЧЕГО НЕТ, БУДУТ ПОКАЗАНЫ ТОЛЬКО ЭКИПИРОВАННЫЕ ПРЕДМЕТЫ|r", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
...
bool ItemUpgrade::TakePagedDataAction(Player* player, Creature* creature, uint32 action)
{
    ...
    if (pagedData.type == PAGED_DATA_TYPE_ITEMS)
    {
        ...
        if (item == nullptr)
            SendMessage(player, "Предмет больше недоступен для улучшения.");
        else
        {
        ...
        if (identifier != nullptr)
        {
            ...
            else
            {
                ...
                if (upgradeStat == nullptr)
                    SendMessage(player, "Улучшение больше недоступно.");
                else
                {
                    ...
                    if (playerUpgrade != nullptr)
                    {
                        ...
                        {
                            SendMessage(player, "Уже достиг МАКСИМУМ ранга для этого параметра и предмета.");
                            BuildStatsUpgradeCatalogue(player, item);
                    ...
                    {
                        SendMessage(player, "Этот ранг недоступен для " + ItemLink(player, item));
                        BuildStatsUpgradeCatalogue(player, item);
                        return AddPagedData(player, creature, pagedData.currentPage);
                    }
    ...
    else if (pagedData.type == PAGED_DATA_TYPE_REQS)
    {
        ...
        else
        {
            if (!PurchaseUpgrade(player))
                SendMessage(player, "Не удалось выполнить улучшение. Этого не должно происходить, если только предмет больше не доступен.");
            else
            {
    ...
    else if (pagedData.type == PAGED_DATA_TYPE_UPGRADED_ITEMS)
    {
        Item* item = FindItemIdentifierFromPage(pagedData, action, player);
        if (item == nullptr)
            SendMessage(player, "Предмет больше не доступен.");
        else
        {
        ...
        else
        {
            ...
            else if (action == 2)
            {
                if (!PurchaseUpgradeBulk(player))
                    SendMessage(player, "Не удалось обработать улучшение. Этого не должно произойти, если только товар больше не доступен.");
                else
                {
                    CloseGossipMenuFor(player);
                    return true;
    ...
    else if (pagedData.type == PAGED_DATA_TYPE_WEAPON_UPGRADE_ITEMS)
    {
        Item* item = FindItemIdentifierFromPage(pagedData, action, player);
        if (item == nullptr)
            SendMessage(player, "Оружие больше недоступно для улучшения.");
        else
        {
    ...
    else if (pagedData.type == PAGED_DATA_TYPE_WEAPON_UPGRADE_PERCS)
    {
        ...
        if (identifier != nullptr && identifier->GetType() == FLOAT_IDENTIFIER)
        {
            ...
            if (!IsValidWeaponForUpgrade(item, player))
                SendMessage(player, "Оружие больше недоступно.");
            else
            {
                auto rebuildPage = [&]()
                {
                ...
                if (weaponUpgrade != nullptr)
                {
                    ...
                    {
                        SendMessage(player, "Вы уже купили это улучшение для оружия!");
                        return rebuildPage();
                    }
                    else
                    {
                        ...
                        if (nextWeaponUpgrade != nullptr)
                        {
                            ...
                            {
                                SendMessage(player, "Сначала нужно купить предыдущее улучшение!");
                                return rebuildPage();
                            }
                            else
                            {
        ...
        else
        {
            ...
            else if (action == 1)
            {
                if (!PurchaseWeaponUpgrade(player))
                    SendMessage(player, "Не удалось обработать улучшение. Этого не должно произойти, если только оружие больше недоступно.");
                else
                {
                    CloseGossipMenuFor(player);
                    return true;
    ...
    else if (pagedData.type == PAGED_DATA_TYPE_WEAPON_UPGRADE_ITEMS_CHECK)
    {
        Item* item = FindItemIdentifierFromPage(pagedData, action, player);
        if (item == nullptr)
            SendMessage(player, "Оружие больше недоступно для улучшений.");
        else
        {
    ...
    else if (pagedData.type == PAGED_DATA_TYPE_WEAPON_SPEED_ITEMS)
    {
        Item* item = FindItemIdentifierFromPage(pagedData, action, player);
        if (item == nullptr)
            SendMessage(player, "Оружие больше недоступно для улучшения скорости.");
        else
        {
        ...
        if (identifier != nullptr && identifier->GetType() == FLOAT_IDENTIFIER)
        {
            ...
            else
            {
                ...
                if (weaponUpgrade != nullptr)
                {
                    if (weaponUpgrade->statModPct >= floatIdentifier->modPct)
                    {
                        SendMessage(player, "Вы уже купили это улучшение скорости оружия!");
                        return rebuildPage();
                    }
                    else
                    {
...
bool ItemUpgrade::PurchaseUpgrade(Player* player)
{
    ...
    {
        SendMessage(player, "Вы не соответствуете требованиям для покупки этого улучшения.");
        return true;
    }
    ...
    VisualFeedback(player);
    SendMessage(player, "Предмет успешно улучшен!");
...
bool ItemUpgrade::PurchaseWeaponUpgrade(Player* player)
{
    ...
    VisualFeedback(player);
    SendMessage(player, "Оружие успешно улучшено!");
...
bool ItemUpgrade::PurchaseWeaponSpeedUpgrade(Player* player)
{
    ...
    VisualFeedback(player);
    SendMessage(player, "Скорость оружия успешно улучшена!");
...
bool ItemUpgrade::PurchaseUpgradeBulk(Player* player)
{
    ...
    {
        SendMessage(player, "Вы не соответствуете требованиям для покупки этих улучшений.");
        return true;
    }
        ...
        for (const auto& req : *reqs)
        {
            ...
            switch (req.reqType)
            {
                case REQ_TYPE_COPPER:
                    oss << "МОНЕТ: " << CopperToMoneyStr((uint32)req.reqVal1, true);
                    break;
                case REQ_TYPE_HONOR:
                    oss << "ЧЕСТЬ: " << (uint32)req.reqVal1 << " points";
                    break;
                case REQ_TYPE_ARENA:
                    oss << "АРЕНА: " << (uint32)req.reqVal1 << " points";
                    break;
                case REQ_TYPE_ITEM:
                {
            ...
            if (!MeetsRequirement(player, req))
            {
                ...
                {
                    case REQ_TYPE_COPPER:
                        missing = "потерянно " + CopperToMoneyStr((uint32)req.reqVal1 - player->GetMoney(), true);
                        break;
                    case REQ_TYPE_HONOR:
                        missing = "потерянно " + Acore::ToString<uint32>((uint32)req.reqVal1 - player->GetHonorPoints()) + " points";
                        break;
                    case REQ_TYPE_ARENA:
                        missing = "потерянно " + Acore::ToString<uint32>((uint32)req.reqVal1 - player->GetArenaPoints()) + " points";
                        break;
                    case REQ_TYPE_ITEM:
                        missing = "потерянно " + Acore::ToString<uint32>((uint32)req.reqVal2 - player->GetItemCount((uint32)req.reqVal1, true)) + " items";
                        break;
                }
            }
            ...
            oss << " - ";
            if (missing.empty())
                oss << "|cff056e3aСДЕЛАНО|r";
            else
                oss << "|cffb50505В ПРОЦЕССЕ|r" << " - " << missing;
    ...
    if (shouldAdd)
    {
        ...
        if (pagedData.type != PAGED_DATA_TYPE_WEAPON_UPGRADE_ITEMS_CHECK && pagedData.type != PAGED_DATA_TYPE_WEAPON_SPEED_UPGRADE_ITEMS_CHECK)
        {
            if (!IsAllowedItem(item) || IsBlacklistedItem(item))
                itemIdentifier->uiName += " [|cffb50505НЕ АКТИВНО|r]";
        }
...
void ItemUpgrade::BuildItemUpgradeStatsCatalogue(const Player* player, const Item* item)
{
    ...
    if (!itemUpgrades.empty())
    {
        ...
        for (const UpgradeStat* upgradeStat : itemUpgrades)
        {
            ...
            std::ostringstream oss;
            oss << "УЛУЧШЕНО " << statTypeStr << " [РАНГ " << upgradeStat->statRank << "]";
...
void ItemUpgrade::_BuildWeaponPercentUpgradesCatalogue(const Player* player, const Item* item, PagedDataType type)
{
    ...
    for (size_t i = 0; i < upgradeStats.size(); i++)
    {
        ...
        if (toPurchase)
            ...
            oss << "|cffb50505";
        oss << "Улучшенно на " << identifier->modPct << "%|r";
        if (toPurchase)
            oss << " [КУПИТЬ]";
        else if (purchased)
            oss << " [СДЕЛАНО]";
...
void ItemUpgrade::BuildWeaponUpgradesPercentInfoCatalogue(const Player* player, const Item* item, float pct)
{
    ...
    pctIdnt->id = 0;
    pctIdnt->uiName = "Повышение урона на " + FormatFloat(pct) + "%";
    pagedData.data.push_back(pctIdnt);

    Identifier* identifier = new Identifier();
    identifier->id = 0;
    identifier->uiName = "Требования:";
    pagedData.data.push_back(identifier);
    ...
    minDmgIdnt->id = 0;
    minDmgIdnt->uiName = "МИН УРОН " + FormatIncrease(currentMinDamage, nextMinDamage);
    pagedData.data.push_back(minDmgIdnt);

    Identifier* maxDmgIdnt = new Identifier();
    maxDmgIdnt->id = 0;
    maxDmgIdnt->uiName = "МАКС УРОН " + FormatIncrease(currentMaxDamage, nextMaxDamage);
...
void ItemUpgrade::BuildWeaponSpeedUpgradesPercentInfoCatalogue(const Player* player, const Item* item, float pct)
{
    ...
    Identifier* pctIdnt = new Identifier();
    pctIdnt->id = 0;
    pctIdnt->uiName = "Повышение скорости на " + FormatFloat(pct) + "% (уменьшит задержку при взмахе оружием)";
    pagedData.data.push_back(pctIdnt);
    ...
    origDelayIdnt->id = 0;
    origDelayIdnt->uiName = "ОРИГИНАЛЬНАЯ СКОРОСТЬ: " + FormatDelay(originalDelay);
    ...
    currentDelayIdnt->uiName = "ТЕКУЩАЯ СКОРОСТЬ: |cffb50505" + FormatDelay(currentDelay) + "|r";
    ...
    nextDelayIdnt->id = 0;
    nextDelayIdnt->uiName = "УМЕНЬШИТСЯ ДО: |cff056e3a" + FormatDelay(nextDelay) + "|r";
...
void ItemUpgrade::BuildWeaponUpgradeInfoCatalogue(const Player* player, const Item* item)
{
    ...
    idnt->id = 0;
    idnt->name = "0";
    idnt->uiName = "Урон улучшится на " + FormatFloat(weaponUpgrade->statModPct) + "%";
    ...
    minDmgIdnt->id = 0;
    minDmgIdnt->name = "1";
    minDmgIdnt->uiName = "МИН УРОН " + FormatIncrease(dmgInfo.first, upgradedDmgInfo.first);
    pagedData.data.push_back(minDmgIdnt);

    Identifier* maxDmgIdnt = new Identifier();
    maxDmgIdnt->id = 0;
    maxDmgIdnt->name = "2";
    maxDmgIdnt->uiName = "МАКС УРОН " + FormatIncrease(dmgInfo.second, upgradedDmgInfo.second);
    pagedData.data.push_back(maxDmgIdnt);
    ...
    if (!item->IsEquipped())
    {
        ...
        equipIdnt->id = 2;
        equipIdnt->name = "3";
        equipIdnt->uiName = "[ЭКИПИРОВАТЬ ПРЕДМЕТ]";
...
void ItemUpgrade::BuildWeaponSpeedUpgradeInfoCatalogue(const Player* player, const Item* item)
{
    ...
    speedIdnt->id = 0;
    speedIdnt->name = "0";
    speedIdnt->uiName = "СКОРОСТЬ УЛУЧШИТСЯ НА " + FormatFloat(weaponUpgrade->statModPct) + "%";
    ...
    origDelayIdnt->uiName = "НАЧАЛЬНАЯ СКОРОСТЬ: " + FormatDelay(originalDelay);
    pagedData.data.push_back(origDelayIdnt);
    ...
    newDelayIdnt->id = 0;
    newDelayIdnt->name = "2";
    newDelayIdnt->uiName = "ТЕКУЩАЯ СКОРОСТЬ: " + FormatDelay(newDelay);
...
void ItemUpgrade::BuildStatsUpgradeCatalogue(const Player* player, const Item* item)
{
    ...
    if (IsAllowedItem(item) && !IsBlacklistedItem(item))
    {
        ...
        for (const UpgradeStat& stat : upgradeStatList)
        {
            ...
            Identifier* identifier = new Identifier();
            std::ostringstream oss;
            oss << "УЛУЧШЕНИЕ " << StatTypeToString(statInfo->ItemStatType) << " ";
            if (foundUpgrade != nullptr)
            {
                ...
                {
                    oss << "[РАНГ " << foundUpgrade->statRank << " |cffb50505МАКС|r]";
                    identifier->id = foundUpgrade->statId;
                    atMaxRank = true;
                }
                else
                {
                    oss << "[РАНГ " << foundUpgrade->statRank << " -> " << "|cff056e3a" << foundUpgrade->statRank + 1 << "|r" << "]";
            ...
            oss << " " << "[" << foundUpgrade->statModPct << "% увеличение - ";
            oss << "|cffb50505" << statInfo->ItemStatValue << "|r --> ";
            oss << "|cff056e3a" << CalculateModPct(statInfo->ItemStatValue, foundUpgrade) << "|r]";
            if (currentUpgrade != nullptr)
            {
                oss << " [ТЕКУЩИЙ: " << CalculateModPct(statInfo->ItemStatValue, currentUpgrade);
            ...
            if (!atMaxRank && !CanApplyUpgradeForItem(item, foundUpgrade))
                oss << " [|cffb50505ОБНОВЛЕНИЕ ЗАПРЕЩЕНО|r]";
...
void ItemUpgrade::BuildStatsUpgradeCatalogueBulk(const Player* player, const Item* item)
{
    ...
    if (IsAllowedItem(item) && !IsBlacklistedItem(item))
    {
        for (const auto& upair : upgradesPctMap)
        {
            ...
            identifier->uiName = "Увеличить ВСЕ характеристики на " + FormatFloat(upair.first) + "%";
            pagedData.data.push_back(identifier);
        }
    }
...
void ItemUpgrade::BuildStatsUpgradeByPctCatalogueBulk(const Player* player, const Item* item, float pct)
{
    ...
    if (upgradesPctMap.find(pct) != upgradesPctMap.end())
    {
        ...
        for (const UpgradeStat* stat : upgrades)
        {
            ...
            if (!IsAllowedStatType(stat->statType))
                oss << "|cffb50505Невозможно|r улучшить " << statTypeStr << ": характеристика недоступна для улучшения";
            else if (!CanApplyUpgradeForItem(item, stat))
                oss << "|cffb50505Невозможно|r улучшить " << statTypeStr << ": недопустимый ранг для этого предмета";
            else
            {
                ...
                if (currentUpgrade != nullptr)
                {
                    ...
                    if (nextUpgrade == nullptr)
                        oss << "|cffb50505Невозможно|r улучшить " << statTypeStr << ": уже максимальный ранг";
                    else
                    {
                        if (currentUpgrade->statRank == stat->statRank - 1)
                            willUpgrade = true;
                        else if (currentUpgrade->statRank >= stat->statRank)
                            oss << "|cffb50505Невозможно|r улучшить " << statTypeStr << ": ранг уже получен";
                        else
                            oss << "|cffb50505Невозможно|r улучшить " << statTypeStr << ": необходимо получить предыдущий(е) ранг(и)";
                    }
                }
                else
                {
                ...
                if (willUpgrade)
                {
                    oss << "|cff056e3aНевозможно|r улучшить " << statTypeStr << " до ранга " << stat->statRank;
                    oss << " [" << FormatFloat(stat->statModPct) << "% вверх, ";
                    oss << "|cffb50505" << foundStat->ItemStatValue << "|r --> ";
                    oss << "|cff056e3a" << CalculateModPct(foundStat->ItemStatValue, stat) << "|r]";

                    if (currentUpgrade != nullptr)
                        oss << " [ТЕКУЩИЙ: " << CalculateModPct(foundStat->ItemStatValue, currentUpgrade) << "]";
                }
            }
...
/*static*/ std::string ItemUpgrade::StatTypeToString(uint32 statType)
{
    static std::unordered_map<uint32, std::string> statTypeToStrMap =
    {
        ...
        {ITEM_MOD_STRENGTH, "Сила"}, {ITEM_MOD_INTELLECT, "Интелект"}, {ITEM_MOD_SPIRIT, "Дух"},
        {ITEM_MOD_STAMINA, "Выносливость"}, {ITEM_MOD_DEFENSE_SKILL_RATING, "Показатель защиты"}, {ITEM_MOD_DODGE_RATING, "Показатель уклонения"},
        {ITEM_MOD_PARRY_RATING, "Показатель парирования"}, {ITEM_MOD_BLOCK_RATING, "Рейтинг блокирования"}, {ITEM_MOD_HIT_MELEE_RATING, "Рейтинг точности"},
        {ITEM_MOD_HIT_RANGED_RATING, "Рейтинг попадания в дальнем бою"}, {ITEM_MOD_HIT_SPELL_RATING, "Рейтинг меткости заклинаний"}, {ITEM_MOD_CRIT_MELEE_RATING, "Рейтинг критического удара в ближнем бою"},
        {ITEM_MOD_CRIT_RANGED_RATING, "Рейтинг критического удара в дальнем бою"}, {ITEM_MOD_CRIT_SPELL_RATING, "Рейтинг критического удара заклинаниями"}, {ITEM_MOD_HIT_TAKEN_MELEE_RATING, "Показатель защиты от ударов в ближнем бою"},
        {ITEM_MOD_HIT_TAKEN_RANGED_RATING, "Рейтинг получаемых попаданий в дистанционных атаках"}, {ITEM_MOD_HIT_TAKEN_SPELL_RATING, "Рейтинг полученного урона от заклинаний"}, {ITEM_MOD_CRIT_TAKEN_MELEE_RATING, "Рейтинг получаемого критического урона в ближнем бою"},
        {ITEM_MOD_CRIT_TAKEN_RANGED_RATING, "Показатель вероятности получения критического удара в дальнем бою"}, {ITEM_MOD_CRIT_TAKEN_SPELL_RATING, "Показатель вероятности получения критического удара заклинанием"}, {ITEM_MOD_HASTE_MELEE_RATING, "Показатель скорости атаки в ближнем бою"},
        {ITEM_MOD_HASTE_RANGED_RATING, "Рейтинг скорости для дальнего боя"}, {ITEM_MOD_HASTE_SPELL_RATING, "Рейтинг скорости заклинаний"}, {ITEM_MOD_HIT_RATING, "Рейтинг меткости"},
        {ITEM_MOD_CRIT_RATING, "Рейтинг критического удара"}, {ITEM_MOD_HIT_TAKEN_RATING, "Рейтинг полученного уронаg"}, {ITEM_MOD_CRIT_TAKEN_RATING, "Показатель вероятности получения критического удара"},
        {ITEM_MOD_RESILIENCE_RATING, "Рейтинг устойчивости"}, {ITEM_MOD_HASTE_RATING, "Рейтинг скорости"}, {ITEM_MOD_EXPERTISE_RATING, "Компетенция"},
        {ITEM_MOD_ATTACK_POWER, "Сила атаки"}, {ITEM_MOD_RANGED_ATTACK_POWER, "Сила атаки в дальнем бою"}, {ITEM_MOD_MANA_REGENERATION, "Регенерация маны"},
        {ITEM_MOD_ARMOR_PENETRATION_RATING, "Пробитие брони"}, {ITEM_MOD_SPELL_POWER, "Сила заклинаний"}, {ITEM_MOD_HEALTH_REGEN, "Регенерация здоровья"},
        {ITEM_MOD_SPELL_PENETRATION, "Пробивание магического сопротивления"}, {ITEM_MOD_BLOCK_VALUE, "Показатель защиты"}
    };
...
/*static*/ std::string ItemUpgrade::EquipmentSlotToString(EquipmentSlots slot)
{
    ...
    {
        {EQUIPMENT_SLOT_START, "Голова"},
        {EQUIPMENT_SLOT_HEAD, "Голова"},
        {EQUIPMENT_SLOT_NECK, "Шея"},
        {EQUIPMENT_SLOT_SHOULDERS, "Плечи"},
        {EQUIPMENT_SLOT_BODY, "Рубашка"},
        {EQUIPMENT_SLOT_CHEST, "Грудь"},
        {EQUIPMENT_SLOT_WAIST, "Пояс"},
        {EQUIPMENT_SLOT_LEGS, "Ноги"},
        {EQUIPMENT_SLOT_FEET, "Ступни"},
        {EQUIPMENT_SLOT_WRISTS, "Запястья"},
        {EQUIPMENT_SLOT_HANDS, "Перчатки"},
        {EQUIPMENT_SLOT_FINGER1, "Палец 1"},
        {EQUIPMENT_SLOT_FINGER2, "Палец 2"},
        {EQUIPMENT_SLOT_TRINKET1, "Аксессуар 1"},
        {EQUIPMENT_SLOT_TRINKET2, "Аксессуар 2"},
        {EQUIPMENT_SLOT_BACK, "Спина"},
        {EQUIPMENT_SLOT_MAINHAND, "Основная рука"},
        {EQUIPMENT_SLOT_OFFHAND, "Вторая рука"},
        {EQUIPMENT_SLOT_RANGED, "Дальний бой"},
        {EQUIPMENT_SLOT_TABARD, "Накидка"}
    };
...
bool ItemUpgrade::TryAddItem(Player* player, uint32 entry, uint32 count, bool add)
{
    ...
    if (proto != nullptr)
    {
        ...
        {
            std::ostringstream oss;
            oss << "Не удалось добавить " << count << "x " << ItemLink(player, proto, 0);
            oss << " проверьте свободное место в инвентаре и попробуйте снова.";
    ...
    for (const UpgradeStatReq& r : reqs)
    {
        switch (r.reqType)
        {
            case REQ_TYPE_COPPER:
                ...
                {
                    SendMessage(player, "Возврата меди не будет, так как будет достигнут лимит золота.");
                    return false;
                }
                break;
            case REQ_TYPE_ITEM:
...
bool ItemUpgrade::AddUpgradeForNewItem(Player* player, Item* item, const UpgradeStat* upgrade, const _ItemStat* stat)
{
    ...
    std::ostringstream oss;
    oss << "|cffeb891a[СИСТЕМА УЛУЧШЕНИЯ ПРЕДМЕТОВ]:|r";
    oss << " " << ItemLink(player, item);
    oss << " статистика " << StatTypeToString(upgrade->statType) << " улучшено до РАНГА " << upgrade->statRank << ".";
    oss << " Увеличинно до " << upgrade->statModPct << "% [";
    oss << stat->ItemStatValue << " --> " << CalculateModPct(stat->ItemStatValue, upgrade) << "]";
    std::pair<uint32, uint32> itemLevel = CalculateItemLevel(player, item);
    oss << " [НОВЫЙ УР.: " << itemLevel.second << "]";
    SendMessage(player, oss.str());
...
/*static*/ std::string ItemUpgrade::FormatItemLocation(const Player* player, const Item* item)
{
    ...
    if (bagSlot == INVENTORY_SLOT_BAG_0)
    {
        ...
        if (itemSlot >= INVENTORY_SLOT_ITEM_START && itemSlot < INVENTORY_SLOT_ITEM_END)
            return "в сумке — ячейка " + Acore::ToString(itemSlot - INVENTORY_SLOT_ITEM_START + 1);
        if (itemSlot >= BANK_SLOT_ITEM_START && itemSlot < BANK_SLOT_ITEM_END)
            return "в банке — ячейка " + Acore::ToString(itemSlot - BANK_SLOT_ITEM_START + 1);
    }
    else
    {
        if (bagSlot >= INVENTORY_SLOT_BAG_START && bagSlot < INVENTORY_SLOT_BAG_END)
            return "в сумке " + Acore::ToString(bagSlot - INVENTORY_SLOT_BAG_START + 1) + " - ячейка " + Acore::ToString(itemSlot + 1);
        if (bagSlot >= BANK_SLOT_BAG_START && bagSlot < BANK_SLOT_BAG_END)
            return "в банке " + Acore::ToString(bagSlot - BANK_SLOT_BAG_START + 1) + " - ячейка " + Acore::ToString(itemSlot + 1);
    }
