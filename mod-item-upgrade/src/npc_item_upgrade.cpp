...
private:
    ...
    static Item* GetPagedDataItem(const ItemUpgrade::PagedData& pagedData, Player* player)
    {
        ...
        if (!validItem)
        {
            ItemUpgrade::SendMessage(player, "Этот предмет больше недоступен для обновления.");
            return nullptr;
        }
    ...
    {
        ClearGossipMenuFor(player);
        AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Выберите оружие, чтобы повысить его скорость.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 12);
        AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "Посмотреть улучшенное оружие", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 13);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<- [Назад]", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
        SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
        return true;
    }
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        {
            ItemUpgrade::SendMessage(player, "Данные об улучшении предметов перезагружаются администратором. Пожалуйста, повторите попытку позже.");
        ...
        if (!sItemUpgrade->GetBoolConfig(CONFIG_ITEM_UPGRADE_ENABLED))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|cffb50505НЕДОСТУПНО|r", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
        else
        {
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "Выберите предмет для улучшения (по характеристикам, по одному параметру)", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
            ...
            if (sItemUpgrade->GetBoolConfig(CONFIG_ITEM_UPGRADE_ALLOW_PURGE))
                AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "Очистить улучшения", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 6);
            ...
            if (sItemUpgrade->GetBoolConfig(CONFIG_ITEM_UPGRADE_WEAPON_DAMAGE))
                AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "[Система повышения урона оружия] ->", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 8);
            if (sItemUpgrade->GetBoolConfig(CONFIG_ITEM_UPGRADE_WEAPON_SPEED))
                AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "[Система повышения скорости оружия] ->", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 11);
            ...
            if (player->GetSession()->GetSecurity() == SEC_ADMINISTRATOR)
                AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "Блокировка редактирования базы данных", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 5, "NPC будет недоступен для игроков до тех пор, пока вы не снимите блокировку с помощью команды .item_upgrade reload.", 0, false);
        }
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Неважно...", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
        return true;
    }
