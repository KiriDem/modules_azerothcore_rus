...
private:
    static bool HandleReloadModItemUpgrade(ChatHandler* handler)
    {
        ...
        handler->SendGlobalGMSysMessage("Данные модуля улучшения предметов успешно перезагружены.");
        return true;
    }
    ...
    {
        sItemUpgrade->SetReloading(true);
        handler->SendSysMessage("NPC, отвечающий за улучшение предметов, теперь заблокирован, редактирование таблиц базы данных безопасно. Снимите блокировку с помощью команды перезагрузки .item_upgrade.");
        return true;
    }
    ...
    static bool HandleListUpgrades(ChatHandler* handler, Optional<PlayerIdentifier> target)
    {
        ...
        {
            handler->PSendSysMessage("Пожалуйста, попробуйте снова через {} секунд.", (listUpgradesDiffTimer - diff) / 1000);
            return true;
        }
            ...
            if (const Item* item = player->GetItemByPos(INVENTORY_SLOT_BAG_0, i))
            {
                ...
                if (!upgrades.empty() || weaponUpgrade != nullptr || weaponSpeedUpgrade != nullptr)
                {
                    ...
                    if (!upgrades.empty())
                    {
                        ...
                        handler->PSendSysMessage("Найдено улучшений характеристик: {}", upgrades.size());
                        for (const auto* stat : upgrades)
                        {
                            ...
                            if (sItemUpgrade->IsInactiveStatUpgrade(item, stat))
                                statusOss << "|cffb50505НЕАКТИВЕН|r";
                            else
                                statusOss << "|cff056e3aАКТИВЕН|r";
                            handler->PSendSysMessage("{} увеличился на {}% [РАНГ {}] [{}] [{}]", ItemUpgrade::StatTypeToString(stat->statType), stat->statModPct, stat->statRank, oss.str(), statusOss.str());
                        }
                    }
                    if (weaponUpgrade != nullptr)
                    {
                        ...
                        handler->PSendSysMessage("Это оружие улучшено {}%, [МИН УРОН {}], [МАКС УРОН {}] [{}]",
                    ...
                    if (weaponSpeedUpgrade != nullptr)
                    {
                        ...
                        handler->PSendSysMessage("Скорость этого оружия улучшено {}%, [ОРИГ СКОРОСТЬ {}] [НОВ СКОРОСТЬ {}] [{}]",
                    ...
                    handler->SendSysMessage("------------ СЛЕДУЮЩИЙ ПРЕДМЕТ ИЛИ КОНЕЦ ------------");
                }
            }
        }

        if (upgradedItems == 0 && weaponUpgrades == 0)
            handler->PSendSysMessage("{} не имеет никаких улучшений.", player->GetPlayerName());
        else
            handler->PSendSysMessage("{} всего: {} улучшенных предметов, {} улучшенных характеристик, {} улучшенного оружия.", player->GetPlayerName(), upgradedItems, upgradedStats, weaponUpgrades);
