    ...
    for (auto& pd : session->Players)
    {
        ...
        else
        {
            ...
            ChatHandler(p->GetSession()).SendSysMessage(
                "|cFFFF0000[Dungeon Master]|r Телепортация не удалась! Возможно, у вас нет доступа к этому подземелью.");
        }
    }
    ...
    for (const auto& pd : session->Players)
        if (Player* p = ObjectAccessor::FindPlayer(pd.PlayerGuid))
        {
            if (session->BossesKilled < session->TotalBosses && p->GetSession())
            {
                char buf[128];
                snprintf(buf, sizeof(buf),
                    "|cFFFFFF00[Dungeon Master]|r Босс повержен! |cFFFFFFFF%u|r remaining.",
...
void DungeonMasterMgr::HandlePlayerDeath(Player* player, Session* session)
{
    ...
    if (session->IsPartyWiped())
    {
        ...
        for (const auto& psd : session->Players)
        {
            ...
            if (p->GetSession())
                ChatHandler(p->GetSession()).SendSysMessage(
                    "|cFFFF0000[Dungeon Master]|r Полная гибель отряда! Испытание провалено.");
    ...
    else
    {
        if (player->GetSession())
            ChatHandler(player->GetSession()).SendSysMessage(
                "|cFFFFFF00[Dungeon Master]|r Вы проиграли! "
                "Вы будете воскрешены, когда ваша группа выйдет из боя.");
    }
}
...
void DungeonMasterMgr::GiveGoldReward(Player* player, uint32 amount)
{
    ...
    if (player->GetSession())
    {
        char buf[128];
        snprintf(buf, sizeof(buf),
            "|cFFFFD700[Dungeon Master]|r Вы получили: |cFFFFD700%u|rg |cFFC0C0C0%u|rs |cFFB87333%u|rc",
...
void DungeonMasterMgr::GiveItemReward(Player* player, uint8 level, uint8 quality)
{
    ...
    if (!itemEntry)
    {
        ...
        if (player->GetSession())
            ChatHandler(player->GetSession()).SendSysMessage(
                "|cFFFF0000[Dungeon Master]|r Для вашего уровня и класса не найдено подходящего снаряжения. Только золото.");
        return;
    }
    ...
    else
    {
        ...
        if (mailItem)
        {
            ...
            MailDraft("Dungeon Master Reward", "Ваши сумки были полны. Вот ваша награда!")
                .AddItem(mailItem)
                .SendMailTo(trans,
            ...
            if (player->GetSession())
                ChatHandler(player->GetSession()).SendSysMessage(
                    "|cFFFFD700[Dungeon Master]|r Сумки полны! Награда отправлена ​​на почту.");
        }
        else
        {
    ...
    if (mailItem)
    {
        ...
        if (const ItemTemplate* t = sObjectMgr->GetItemTemplate(itemEntry))
        {
            if (player->GetSession())
            {
                char buf[256];
                snprintf(buf, sizeof(buf),
                    "|cFFFFD700[Dungeon Master]|r Нарады отправленны: |cFFFFFFFF%s|r", t->Name1.c_str());
                ChatHandler(player->GetSession()).SendSysMessage(buf);
            }
        }
    }
                                                    ...
                                                    const std::vector<ObjectGuid>& playerGuids)
{
    ...
    for (const auto& guid : playerGuids)
    {
        ...
        if (p->GetSession())
            ChatHandler(p->GetSession()).SendSysMessage(
                "|cFF00FFFF[Roguelike]|r Награды добавлены в ваш инвентарь!");
    }
}
...
void DungeonMasterMgr::EndSession(uint32 sessionId, bool success)
{
    ...
    for (const auto& pd : s.Players)
        if (Player* p = ObjectAccessor::FindPlayer(pd.PlayerGuid))
            if (p->GetSession())
                ChatHandler(p->GetSession()).SendSysMessage(
                    success ? "|cFF00FF00[Dungeon Master]|r Испытание завершено! Выдача наград..."
                            : "|cFFFF0000[Dungeon Master]|r Испытание завершено. Награды не выданы.");
                    ...
                    if (session.TotalMobs == 0 && session.TotalBosses == 0)
                    {
                        ...
                        if (m && m->IsDungeon())
                        {
                            ...
                            if (inst)
                            {
                                ...
                                for (const auto& pd2 : session.Players)
                                    if (Player* p2 = ObjectAccessor::FindPlayer(pd2.PlayerGuid))
                                        ChatHandler(p2->GetSession()).SendSysMessage(
                                            "|cFF00FF00[Dungeon Master]|r Подготовка к испытанию...");
                                ...
                                char buf[256];
                                snprintf(buf, sizeof(buf),
                                    "|cFF00FF00[Dungeon Master]|r |cFFFFFFFF%u|r порожденно "
                                    "|cFFFFFFFF%u|r врагов и босов. Уровень существ: "
                                    ...
                                    if (Player* p3 = ObjectAccessor::FindPlayer(pd3.PlayerGuid))
                                        if (p3->GetSession())
                                            ChatHandler(p3->GetSession()).SendSysMessage(
                                                "|cFFFF8000[Dungeon Master]|r Босс входит в новую фазу!");
                                    ...
                                    if (Player* p = ObjectAccessor::FindPlayer(pd2.PlayerGuid))
                                        if (p->GetSession())
                                        {
                                            ...
                                            snprintf(buf, sizeof(buf),
                                                ...
                                                "Награда через |cFFFFFFFF%u|r секунд...",
                                                session.RoguelikeRunId != 0
                                                    ? "Этаж зачищен!" : "Подземелье пройденно!",
                                                delay);
                                            ChatHandler(p->GetSession()).SendSysMessage(buf);
                                        }
                                break;
                ...
                if (session.IsActive() && !session.IsGroupInCombat())
                {
                    for (const auto& pd : session.Players)
                    {
                        ...
                        if (p && !p->IsAlive() && p->GetMapId() == session.MapId)
                        {
                            ...
                            ChatHandler(p->GetSession()).SendSysMessage(
                                "|cFF00FF00[Dungeon Master]|r Возрожден на входе. Возвращайся обратно!");
                        }
                    }
                }
            }
