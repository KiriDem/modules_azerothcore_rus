...
private:
    ...
    void ShowDifficultyMenu(Player* player, Creature* creature)
    {
        ...
        for (const auto& d : sDMConfig->GetDifficulties())
        {
            ...
            if (!d.IsValidForLevel(lvl))
                snprintf(buf, sizeof(buf), "|cFF808080%s|r (Ур %u-%u) - |cFFFF0000Requires %u+|r",
                    d.Name.c_str(), d.MinLevel, d.MaxLevel, d.MinLevel);
            else if (!d.IsOnLevelFor(lvl))
                snprintf(buf, sizeof(buf), "%s |cFF808080(Ур %u-%u — Easy)|r",
    ...
    {
        player->PlayerTalkClass->ClearMenus();
        ChatHandler(player->GetSession()).SendSysMessage("|cFFFFD700========= Испытание для Мастера подземелий =========|r");
    ...
    void ShowNormalStats(Player* player, Creature* creature)
    {
        ...
        snprintf(buf, sizeof(buf), "  Забегов: |cFFFFFFFF%u|r  —  Завершенно: |cFF00FF00%u|r  —  Проваленно: |cFFFF0000%u|r",
    ...
    void ShowRoguelikeStats(Player* player, Creature* creature)
    {
        ...
        if (st.TotalRuns > 0)
        {
            ...
            snprintf(buf, sizeof(buf), "  Среднее кол-во этажей за забег: |cFF00FFFF%.1f|r", avgFloors);
            chat.SendSysMessage(buf);
        }
        ...
        if (st.LongestRunTime > 0)
        {
            ...
            snprintf(buf, sizeof(buf), "  Самый длинный забег: |cFF00FFFF%s|r", timeBuf);
            chat.SendSysMessage(buf);
        }
    ...
    void ShowNormalLeaderboard(Player* player, Creature* creature)
    {
        ...
        chat.SendSysMessage("|cFFFFD700═══════ Обычные пробежки — самые быстрые зачистки ═══════|r");

        if (entries.empty())
        {
            chat.SendSysMessage("  |cFF808080Пока ни одного забега не зафиксировано.|r");
        }
        else
        {
            uint32 rank = 0;
    ...
    void ShowRoguelikeDifficultyMenu(Player* player, Creature* creature)
    {
        ...
        ChatHandler(player->GetSession()).SendSysMessage(
            "|cFFFF0000Один провал завершает забег!|r");
        ChatHandler(player->GetSession()).SendSysMessage(
            "|cFF00FFFF========================================|r");
    ...
    void StartRoguelike(Player* player, Creature* /*creature*/)
    {
        ...
        {
            ChatHandler(player->GetSession()).SendSysMessage(
                "|cFFFF0000[Roguelike]|r Не удалось начать забег в стиле рогалик!");
            return;
        }

        ChatHandler(player->GetSession()).SendSysMessage(
            "|cFF00FFFF[Roguelike]|r Забег начат! Зачищайте подземелья, чтобы продвигаться дальше. Удачи!");
    }
    ...
    void StartChallenge(Player* player, Creature* /*creature*/)
    {
        ...
        if (sDMConfig->ShouldAnnounceCompletion())
        {
            ...
            char buf[256];
            snprintf(buf, sizeof(buf),
                "|cFF00FF00[Dungeon Master]|r |cFFFFFFFF%s|r начал |cFFFFD700%s|r |cFF00FFFF%s|r забег!",
                player->GetName().c_str(), diff->Name.c_str(),
                theme ? theme->Name.c_str() : "Random");

            char detail[256];
            snprintf(detail, sizeof(detail),
                "|cFFFFD700[Dungeon Master]|r Сложность: |cFF00FF00%s|r  Тема: |cFF00FF00%s|r  Подземелье: |cFF00FF00%s|r  Scaling: |cFF00FF00%s|r",
                diff->Name.c_str(),
                theme ? theme->Name.c_str() : "Random",
