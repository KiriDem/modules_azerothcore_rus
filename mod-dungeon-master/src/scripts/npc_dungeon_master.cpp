...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (sRoguelikeMgr->IsPlayerInRun(player->GetGUID()))
        {
            ...
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Неважно",
...
private:
    ...
    {
        player->PlayerTalkClass->ClearMenus();
        AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Начало Испытания",
            GOSSIP_SENDER_MAIN, GOSSIP_ACTION_MAIN_START);
        if (sDMConfig->IsRoguelikeEnabled())
            AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "|cFF00FFFFРежим рогалика|r",
        ...
        AddGossipItemFor(player, GOSSIP_ICON_TABARD, "|cFFFFD700Статистика & Таблица лидеров|r",
    ...
    void ShowDifficultyMenu(Player* player, Creature* creature)
    {
        ...
        for (const auto& d : sDMConfig->GetDifficulties())
        {
            ...
            if (!d.IsValidForLevel(lvl))
                snprintf(buf, sizeof(buf), "|cFF808080%s|r (Ур %u-%u) - |cFFFF0000Требует %u+|r",
                    d.Name.c_str(), d.MinLevel, d.MaxLevel, d.MinLevel);
            else if (!d.IsOnLevelFor(lvl))
                snprintf(buf, sizeof(buf), "%s |cFF808080(Ур %u-%u — Легкий)|r",
    ...
    void ShowScalingMenu(Player* player, Creature* creature)
    {
        ...
        snprintf(buf1, sizeof(buf1),
            "|cFF00FF00Подстроить под уровень группы|r (Ур %u) — Полноценное испытание вашего уровня",
            partyLevel);
        snprintf(buf2, sizeof(buf2),
            "|cFFFFD700Использовать сложность подземелья|r (Ур %u-%u) — Исходный диапазон сложности",
    ...
    void ShowConfirmMenu(Player* player, Creature* creature)
    {
        ...
        char buf[256];
        ChatHandler(player->GetSession()).SendSysMessage("|cFFFFD700========== Обзор Испытания ==========|r");
        snprintf(buf, sizeof(buf), "  Сложность: |cFF00FF00%s|r", diff ? diff->Name.c_str() : "?");
        ChatHandler(player->GetSession()).SendSysMessage(buf);
        snprintf(buf, sizeof(buf), "  Масштабирование:    |cFF00FF00%s|r",
            sel.ScaleToParty ? "Уровень Группы" : "Dungeon Difficulty");
        ChatHandler(player->GetSession()).SendSysMessage(buf);
        ...
        ChatHandler(player->GetSession()).SendSysMessage(buf);
        snprintf(buf, sizeof(buf), "  Размер Группы: |cFFFFFFFF%u|r player(s)", ps);
        ChatHandler(player->GetSession()).SendSysMessage(buf);
        if (ps > 1)
        ...
        AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "|cFF00FF00>> НАЧАЛО ИСПЫТАНИЯ <<|r",
    ...
    void ShowInfoMenu(Player* player, Creature* creature)
    {
        ...
        ChatHandler(player->GetSession()).SendSysMessage("|cFFFFFFFF2.|r Масштабирование добычи: уровень группы или сложность подземелья");
        ChatHandler(player->GetSession()).SendSysMessage("|cFFFFFFFF3.|r Выберите тему, связанную с существами.");
        ChatHandler(player->GetSession()).SendSysMessage("|cFFFFFFFF4.|r Выберите подземелье или случайный вариант.");
        ChatHandler(player->GetSession()).SendSysMessage("|cFFFFFFFF5.|r Вас телепортируют в зачищенный инстанс.");
        ChatHandler(player->GetSession()).SendSysMessage("|cFFFFFFFF6.|r Победите босса, чтобы выполнить испытание.");
        ChatHandler(player->GetSession()).SendSysMessage("|cFFFFFFFF7.|r Собирайте награды в виде золота и снаряжения!");
        ChatHandler(player->GetSession()).SendSysMessage("|cFFFFD700==========================================|r");
    ...
    {
        player->PlayerTalkClass->ClearMenus();
        AddGossipItemFor(player, GOSSIP_ICON_TABARD, "Статистика моих обычных испытаний",
            GOSSIP_SENDER_MAIN, GOSSIP_ACTION_STATS_NORMAL);
        if (sDMConfig->IsRoguelikeEnabled())
            AddGossipItemFor(player, GOSSIP_ICON_TABARD, "|cFF00FFFFМоя статистика в рогалике|r",
                GOSSIP_SENDER_MAIN, GOSSIP_ACTION_STATS_ROGUELIKE);
        AddGossipItemFor(player, GOSSIP_ICON_TABARD, "|cFFFFD700Таблица лидеров|r",
    ...
    void ShowNormalStats(Player* player, Creature* creature)
    {
        ...
        chat.SendSysMessage("|cFFFFD700═══════════ Статистика Обычного Прохождения ═══════════|r");

        snprintf(buf, sizeof(buf), "  Испытаний: |cFFFFFFFF%u|r  —  Завершенно: |cFF00FF00%u|r  —  Проваленно: |cFFFF0000%u|r",
        ...
        snprintf(buf, sizeof(buf), "  Убито мобов:  |cFFFFFFFF%u|r  —  Боссов поверженно: |cFFFFFFFF%u|r",
        ...
        if (st.TotalDeaths > 0 && st.TotalMobsKilled > 0)
        {
            ...
            snprintf(buf, sizeof(buf), "  Соотношение Убийств/Смертей: |cFFFFD700%.1f|r", kd);
            chat.SendSysMessage(buf);
        }
        ...
        if (st.FastestClear > 0)
        {
            ...
            snprintf(buf, sizeof(buf), "  Самое быстрое прохождение: |cFF00FFFF%s|r", timeBuf);
            chat.SendSysMessage(buf);
        }

        chat.SendSysMessage("|cFFFFD700══════════════════════════════════════════|r");

        AddGossipItemFor(player, GOSSIP_ICON_TABARD, "|cFFFFD700Посмотреть таблицу лидеров|r",
            GOSSIP_SENDER_MAIN, GOSSIP_ACTION_BOARD_MENU);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|cFFFF0000<< Назад|r",
            GOSSIP_SENDER_MAIN, GOSSIP_ACTION_STATS_MENU);
        SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
    ...
    void ShowRoguelikeStats(Player* player, Creature* creature)
    {
        ...
        chat.SendSysMessage("|cFF00FFFF═══════════ Статистика Рогаликов ═══════════|r");

        snprintf(buf, sizeof(buf), "  Всего испытаний: |cFFFFFFFF%u|r", st.TotalRuns);
        chat.SendSysMessage(buf);

        snprintf(buf, sizeof(buf), "  Мак. уровень: |cFFFFD700%u|r  —  Большинства этажей: |cFFFFD700%u|r",
            st.HighestTier, st.MostFloorsCleared);
        chat.SendSysMessage(buf);

        snprintf(buf, sizeof(buf), " Всего зачищено этажей: |cFFFFFFFF%u|r", st.TotalFloorsCleared);
        chat.SendSysMessage(buf);

        chat.SendSysMessage(" ");

        snprintf(buf, sizeof(buf), "  Мобов убито: |cFFFFFFFF%u|r  —  Боссов поверженно: |cFFFFFFFF%u|r",
            st.TotalMobsKilled, st.TotalBossesKilled);
        chat.SendSysMessage(buf);
        ...
        if (st.TotalRuns > 0)
        {
            ...
            snprintf(buf, sizeof(buf), "  Среднее кол-во этажей за испытание: |cFF00FFFF%.1f|r", avgFloors);
            chat.SendSysMessage(buf);
        }
        ...
        if (st.LongestRunTime > 0)
        {
            ...
            snprintf(buf, sizeof(buf), "  Самое длинное испытание: |cFF00FFFF%s|r", timeBuf);
            chat.SendSysMessage(buf);
        }
    ...
    void ShowBoardMenu(Player* player, Creature* creature)
    {
        ...
        AddGossipItemFor(player, GOSSIP_ICON_TABARD,
            "|cFFFFD700Обычные пробежки — самые быстрые зачистки|r",
            GOSSIP_SENDER_MAIN, GOSSIP_ACTION_BOARD_NORMAL);
        if (sDMConfig->IsRoguelikeEnabled())
        {
            AddGossipItemFor(player, GOSSIP_ICON_TABARD,
                "|cFF00FFFFРогалик — Высший уровень|r",
                GOSSIP_SENDER_MAIN, GOSSIP_ACTION_BOARD_RL_TIER);
            AddGossipItemFor(player, GOSSIP_ICON_TABARD,
                "|cFF00FFFFРогалик — Высший этаж|r",
    ...
    void ShowNormalLeaderboard(Player* player, Creature* creature)
    {
        ...
        chat.SendSysMessage("|cFFFFD700═══════ Обычные Испытания — Самые Быстрые Зачистки ═══════|r");

        if (entries.empty())
        {
            chat.SendSysMessage("  |cFF808080Пока ни одного испытания не зафиксировано.|r");
        }
        else
        {
            uint32 rank = 0;
    ...
    void ShowRoguelikeLeaderboard(Player* player, Creature* creature, bool sortByFloors)
    {
        ...
        if (sortByFloors)
            chat.SendSysMessage("|cFF00FFFF═══════ Рогалик — Высший Этаж ═══════|r");
        else
            chat.SendSysMessage("|cFF00FFFF═══════ Рогалик — Высший Уровень ═══════|r");
        ...
        else
        {
            ...
            for (const auto& e : entries)
            {
                ...
                char buf[384];
                snprintf(buf, sizeof(buf),
                    "  %s#%u%s |cFFFFFFFF%s|r — Уровень |cFFFFD700%u|r — |cFF00FF00%u|r этаж%s — |cFF00FFFF%s|r — %u убито — %uP%s",
                    isMe ? "|cFF00FF00" : "|cFFFFD700",
                    rank,
                    isMe ? "|r" : "|r",
                    e.CharName.c_str(),
    ...
    void ShowRoguelikeDifficultyMenu(Player* player, Creature* creature)
    {
        ...
        ChatHandler(player->GetSession()).SendSysMessage(
            "|cFF00FFFF========== Режим Рогалика ==========|r");
        ChatHandler(player->GetSession()).SendSysMessage(
            "|cFFFFFFFFЗачищайте подземелья одно за другим. Каждая успешная зачистка повышает уровень сложности.|r");
        ChatHandler(player->GetSession()).SendSysMessage(
            "|cFFFFFFFFВраги становятся сильнее, но вы получаете мощные усиления.|r");
        ChatHandler(player->GetSession()).SendSysMessage(
            "|cFFFF0000Один провал завершает испытание!|r");
        ChatHandler(player->GetSession()).SendSysMessage(
            "|cFF00FFFF========================================|r");
    ...
    void ShowRoguelikeScalingMenu(Player* player, Creature* creature)
    {
        ...
        char buf1[256], buf2[256];
        snprintf(buf1, sizeof(buf1),
            "|cFF00FF00Подстроить под уровень группы|r (Ур %u)", partyLevel);
        snprintf(buf2, sizeof(buf2),
            "|cFFFFD700Использовать сложность подземелья|r — Исходные диапазоны уровней");
    ...
    void StartRoguelike(Player* player, Creature* /*creature*/)
    {
        ...
        { std::lock_guard<std::mutex> lk(sSelMutex);
          ...
          if (it == sSelections.end()) {
              ChatHandler(player->GetSession()).SendSysMessage(
                  "|cFFFF0000[Roguelike]|r Срок выбора истек. Попробуйте еще раз.");
              return; }
          sel = it->second;
        ...
        {
            ChatHandler(player->GetSession()).SendSysMessage(
                "|cFFFF0000[Roguelike]|r Не выполнены требования к уровню!");
            return;
        }
        ...
        {
            ChatHandler(player->GetSession()).SendSysMessage(
                "|cFFFF0000[Roguelike]|r Не удалось начать испытание в стиле рогалик!");
            return;
        }

        ChatHandler(player->GetSession()).SendSysMessage(
            "|cFF00FFFF[Roguelike]|r Испытание началось! Зачищайте подземелья, чтобы продвигаться дальше. Удачи!");
    }
    ...
    void StartChallenge(Player* player, Creature* /*creature*/)
    {
        ...
        { std::lock_guard<std::mutex> lk(sSelMutex);
          auto it = sSelections.find(player->GetGUID());
          if (it == sSelections.end()) {
              ChatHandler(player->GetSession()).SendSysMessage("|cFFFF0000[Dungeon Master]|r Срок выбора истек. Попробуйте еще раз.");
              return; }
          sel = it->second;
          sSelections.erase(it); }
        ...
        if (!diff || !diff->IsValidForLevel(player->GetLevel()))
        {
            ChatHandler(player->GetSession()).SendSysMessage("|cFFFF0000[Dungeon Master]|r Не выполнены требования к уровню!");
            return;
        }
        ...
        if (mapId == 0)
        {
            ...
            if (dgs.empty()) {
                ChatHandler(player->GetSession()).SendSysMessage("|cFFFF0000[Dungeon Master]|r Нет доступных подземелий!");
                return; }
        ...
        if (!s) {
            ChatHandler(player->GetSession()).SendSysMessage("|cFFFF0000[Dungeon Master]|r Не удалось создать сеанс!");
            return; }

        if (!sDungeonMasterMgr->StartDungeon(s)) {
            ChatHandler(player->GetSession()).SendSysMessage("|cFFFF0000[Dungeon Master]|r Не удалось инициализировать подземелье!");
            sDungeonMasterMgr->AbandonSession(s->SessionId); return; }

        if (!sDungeonMasterMgr->TeleportPartyIn(s)) {
            ChatHandler(player->GetSession()).SendSysMessage("|cFFFF0000[Dungeon Master]|r Телепорт не удался!");
            sDungeonMasterMgr->AbandonSession(s->SessionId); return; }
        ...
        if (sDMConfig->ShouldAnnounceCompletion())
        {
            ...
            char buf[256];
            snprintf(buf, sizeof(buf),
                "|cFF00FF00[Dungeon Master]|r |cFFFFFFFF%s|r начал |cFFFFD700%s|r |cFF00FFFF%s|r испытание!",
                player->GetName().c_str(), diff->Name.c_str(),
                theme ? theme->Name.c_str() : "Random");

            char detail[256];
            snprintf(detail, sizeof(detail),
                "|cFFFFD700[Dungeon Master]|r Сложность: |cFF00FF00%s|r  Тема: |cFF00FF00%s|r  Подземелье: |cFF00FF00%s|r  Маштабирование: |cFF00FF00%s|r",
                diff->Name.c_str(),
                theme ? theme->Name.c_str() : "Random",
