...
//        { 668, "Halls of Reflection",  79, 80 },
        ...
        {  36, "Мертвые копи",            15, 25 },
        {  33, "Крепость Темного Клыка",  18, 28 },
        {  34, "Тюрьма",                  20, 30 },
        {  43, "Пещеры Стенаний",         15, 28 },
        {  48, "Непроглядная Пучина",     20, 32 },
        {  47, "Курганы Иглошкурых",      25, 35 },
        {  90, "Гномреган",               25, 38 },
        { 129, "Лабиринты Иглошкурых",    35, 45 },
        { 189, "Монастырь Алого ордена",  30, 45 },
        {  70, "Ульдаман",                38, 50 },
        { 209, "Зул'Фаррак",              42, 52 },
        { 349, "Мародон",                 40, 52 },
        { 109, "Затонувший храм",         45, 55 },
        { 230, "Глубины Черной горы",     48, 60 },
        { 229, "Вершина Черной горы",     52, 60 },
        { 289, "Некроситет",              55, 60 },
        { 329, "Стратхольм",              55, 60 },
        // TBC
        { 543, "Бастионы Адского Пламени", 58, 70 },
        { 542, "Кузня Крови",              59, 70 },
        { 547, "Узилище",                  60, 70 },
        { 546, "Низины",                   61, 70 },
        { 557, "Гробницы маны",            62, 70 },
        { 558, "Аукенайские гробницы",     63, 70 },
        { 556, "Сетеккские залы",          65, 70 },
        { 555, "Темный лабиринт",          68, 70 },
        { 540, "Разрушенные залы",         68, 70 },
        { 553, "Ботаника",                 68, 70 },
        { 554, "Механар",                  68, 70 },
        { 552, "Аркатрац",                 68, 70 },
        // WotLK
...
void DMConfig::LoadRoguelikeBuffPool()
{
    ...
    if (_roguelikeBuffPool.empty())
    {
        ...
        _roguelikeBuffPool.push_back({1, 15366, "Серенада из Цветущих песнопений",   100});  // +15 stats, +5% crit
        _roguelikeBuffPool.push_back({2, 22888, "Призыв к Сплочению",          80});   // +140 AP, +10% spell crit
        _roguelikeBuffPool.push_back({3, 24425, "Дух Зандалара",    80});   // +15% move speed, +10% stats
        _roguelikeBuffPool.push_back({4, 16609, "Благословение Вождя",   80});   // +300 HP, haste, MP5
        _roguelikeBuffPool.push_back({5, 23768, "Удача Ущерба",     60});   // +10% damage

        // Class buffs (work on any target via AddAura)
        _roguelikeBuffPool.push_back({6, 20217, "Благословение Королей",     90});   // +10% all stats
        _roguelikeBuffPool.push_back({7, 48161, "Слово силы: Стойкость", 100});  // +stamina
        _roguelikeBuffPool.push_back({8, 48469, "Дар Дикой природы",      100});  // +stats/armor/resists
        _roguelikeBuffPool.push_back({9, 19506, "Аура Меткого выстрела",         70});   // +10% AP
        _roguelikeBuffPool.push_back({10, 24932, "Вожак Стаи",   70});   // +5% crit
    }
}
