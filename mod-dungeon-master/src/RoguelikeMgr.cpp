...
bool RoguelikeMgr::StartRun(Player* leader, uint32 difficultyId, uint32 themeId, bool scaleToParty)
{
    ...
    if (!sDMConfig->IsRoguelikeEnabled())
    {
        ChatHandler(leader->GetSession()).SendSysMessage(
            "|cFFFF0000[Roguelike]|r Режим рогалика выключен.");
        return false;
    }
    ...
    {
        ChatHandler(leader->GetSession()).SendSysMessage(
            "|cFFFF0000[Roguelike]|r Вы уже находитесь в испытании в стиле рогалика.!");
        return false;
    }
    ...
    {
        ChatHandler(leader->GetSession()).SendSysMessage(
            "|cFFFF0000[Roguelike]|r Вы участвуете в активном испытании подземелья.!");
        return false;
    }
    ...
    if (sDungeonMasterMgr->IsOnCooldown(leader->GetGUID()))
    {
        ...
        snprintf(buf, sizeof(buf),
            "|cFFFF0000[Roguelike]|r До старта ждать |cFFFFFFFF%u|r мин |cFFFFFFFF%u|r сек.",
            rem / 60, rem % 60);
    ...
    {
        ChatHandler(leader->GetSession()).SendSysMessage(
            "|cFFFF0000[Roguelike]|r Слишком много активных заданий. Попробуйте позже.");
        return false;
    }
    ...
    {
        ChatHandler(leader->GetSession()).SendSysMessage(
            "|cFFFF0000[Roguelike]|r Для вашего уровня нет доступных подземелий.!");
        return false;
    }
    ...
    {
        ChatHandler(leader->GetSession()).SendSysMessage(
            "|cFFFF0000[Roguelike]|r Не удалось создать сеанс подземелья!");
        return false;
    }
    ...
    {
        ChatHandler(leader->GetSession()).SendSysMessage(
            "|cFFFF0000[Roguelike]|r Сбой телепортации!");
        sDungeonMasterMgr->CleanupRoguelikeSession(session->SessionId, false);
        return false;
    }
    ...
    char buf[256];
    snprintf(buf, sizeof(buf),
        "|cFF00FFFF[Roguelike]|r |cFFFFD700%s|r Начало испытания в рогалике! "
        "Тема: |cFF00FF00%s|r — Как далеко вы сможете зайти?",
...
void RoguelikeMgr::OnDungeonCompleted(uint32 runId, uint32 sessionId)
{
    ...
    char buf[512];
    snprintf(buf, sizeof(buf),
        "|cFF00FFFF[Roguelike]|r |cFFFFD700Этаж %u зачищен!|r "
        "Переход на следующий |cFFFF0000Этаж %u|r...",
    ...
    if (!TransitionToNextDungeon(*run))
    {
        ...
        snprintf(failBuf, sizeof(failBuf),
            ...
            "Испытание завершилось на |cFFFFD700Уровне %u|r после |cFFFFFFFF%u|r этажей.",
...
void RoguelikeMgr::OnPartyWipe(uint32 runId)
{
    ...
    char buf[512];
    snprintf(buf, sizeof(buf),
        "|cFFFF0000[Roguelike]|r |cFFFF4444ПОЛНОЕ УНИЧТОЖЕНИЕ КОМАНДЫ!|r "
        "Вы проиграли.\n"
        "|cFF00FFFF[Roguelike]|r Финальный результат:\n"
        "  Уровней достигнуто: |cFFFFD700%u|r\n"
        "  Очищенно этажей: |cFFFFFFFF%u|r\n"
        "  Убито существ: |cFFFFFFFF%u|r\n"
        "  Боссов поверженно: |cFFFFFFFF%u|r\n"
        "  Кол-во смертей: |cFFFF0000%u|r\n"
        "  Время испытания: |cFF00FFFF%um %02us|r",
        run->CurrentTier, run->DungeonsCleared,
...
void RoguelikeMgr::EndRun(uint32 runId, bool announceResults)
{
    ...
    if (announceResults)
    {
        ...
        char buf[512];
        snprintf(buf, sizeof(buf),
            "|cFF00FFFF[Roguelike]|r Испытание окончено!\n"
            "  Достигнут уровень: |cFFFFD700%u|r\n"
            "  Зачищенно Этажей: |cFFFFFFFF%u|r\n"
            "  Убито существ: |cFFFFFFFF%u|r\n"
            "  Боссов поверженно: |cFFFFFFFF%u|r\n"
            "  Время испытания: |cFF00FFFF%um %02us|r",
            run->CurrentTier, run->DungeonsCleared,
...
void RoguelikeMgr::IncrementBuffStacks(uint32 runId)
{
    ...
    char buf[256];
    snprintf(buf, sizeof(buf),
        "|cFF00FFFF[Roguelike]|r |cFF00FF00+%.0f%% Статистика|r (Стек %u)",
...
bool RoguelikeMgr::TransitionToNextDungeon(RoguelikeRun& run)
{
    ...
    char buf[256];
    snprintf(buf, sizeof(buf),
        "|cFF00FFFF[Roguelike]|r Входим на |cFFFFFFFF%s|r — Уровень |cFFFF0000%u|r",
...
void RoguelikeMgr::AnnounceCountdown(const RoguelikeRun& run, uint32 remainingSec)
{
    char buf[128];
    snprintf(buf, sizeof(buf),
        "|cFF00FFFF[Roguelike]|r Следующее подземелье через |cFFFFFFFF%u|r секунд...",
