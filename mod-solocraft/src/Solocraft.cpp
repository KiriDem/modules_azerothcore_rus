...
public:
    ...
    void OnPlayerLogin(Player* player) override
    {
        if (SoloCraftEnable && SoloCraftAnnounceModule)
            ChatHandler(player->GetSession()).SendSysMessage("На этом сервере работает модуль |cff4CFF00SoloCraft |r.");
        ...
        else
        {
            ...
            if (player->GetLevel() <= dunLevel + SolocraftLevelDiff)
            {
                ...
                if (difficulty > 0)
                {
                    ...
                    if (!SolocraftXPEnabled)
                    {
                        ss << "|cffFF0000[SoloCraft] |cffFF8000" << player->GetName() << " вошел(а) {}  - Смещение сложности: {:.2f}. Бонус к силе заклинаний: {}. Весовой коэффициент баланса класса: {}.  Получение опыта: |cffFF0000Disabled";
                    ...
                    else
                    {
                        ...
                        {
                            ss << "|cffFF0000[SoloCraft] |cffFF8000" << player->GetName() << " вошел(а) {}  - Смещение сложности: {:.2f}. Бонус к силе заклинаний: {}. Весовой коэффициент баланса класса: {}.  Баланс опыта: |cffFF0000Disabled";
                            ChatHandler(player->GetSession()).PSendSysMessage(ss.str().c_str(), map->GetMapName(), difficulty, SpellPowerBonus, classBalance);
                        }
                        else
                        {
                            ss << "|cffFF0000[SoloCraft] |cffFF8000" << player->GetName() << " вошел(а) {}  - Смещение сложности: {:.2f}. Бонус к силе заклинаний: {}. Весовой коэффициент баланса класса: {}.  Баланс опыта: |cff4CFF00Enabled";
                            ChatHandler(player->GetSession()).PSendSysMessage(ss.str().c_str(), map->GetMapName(), difficulty, SpellPowerBonus, classBalance);
                        }
                    }
                }
                else
                {
                    ...
                    {
                        ss << "|cffFF0000[SoloCraft] |cffFF8000" << player->GetName() << " вошел(а) {}  - |cffFF0000ВНИМАНИЕ! Вам был наложен дебафф со смещением: {:.2f} и весом баланса класса: {}. |cffFF8000 У участника группы, уже находящегося внутри, эффект от баффов подземелья полностью компенсируется. Заклинатели не получат бонус к силе заклинаний. Для получения сбалансированной компенсации все члены группы должны выйти из подземелья и войти в него снова.";
                        ChatHandler(player->GetSession()).PSendSysMessage(ss.str().c_str(), map->GetMapName(), difficulty, classBalance);
                    }
                    else
                    {
                        ss << "|cffFF0000[SoloCraft] |cffFF8000" << player->GetName() << " вошел(а) {}  - |cffFF0000ВНИМАНИЕ! На вас наложен дебафф смещения {:.2f} с весом баланса класса {}, и опыт начисляться не будет. |cffFF8000 У одного из участников группы, уже находящихся внутри, действует полное смещение усиления подземелья. Заклинатели не получат бонус к силе заклинаний. ВСЕ участники группы должны выйти из подземелья и войти снова, чтобы получить сбалансированное значение смещения.";
                        ChatHandler(player->GetSession()).PSendSysMessage(ss.str().c_str(), map->GetMapName(), difficulty, classBalance);
                    }
                }
            ...
            else
            {
                ...
                ss << "|cffFF0000[SoloCraft] |cffFF8000" << player->GetName() << " вошел(а) {}  - |cffFF0000Вы не получили усиления. |cffFF8000 Ваш уровень превышает максимальный порог ({}) для этого подземелья.";
